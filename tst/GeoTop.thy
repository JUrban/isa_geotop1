...
theorem Theorem_GT_9_1:
  ...
  assumes hA: "geotop_is_arc A (subspace_topology UNIV geotop_euclidean_topology A)"
  assumes hB: "geotop_is_broken_line B"
  assumes hJ_ab: "J = A \<union> B" and hAB_int: "A \<inter> B = {P, Q}"
  assumes hR: "R \<in> geotop_arc_interior A {P, Q}" and hS: "S \<in> geotop_arc_interior B {P, Q}"
  assumes hMs_bl: "\<forall>i<n. geotop_is_broken_line (Ms i)"
  assumes hMs_disj: "\<forall>i<n. \<forall>j<n. i \<noteq> j \<longrightarrow> Ms i \<inter> Ms j = {}"
  assumes hM_def: "M = (\<Union>i<n. Ms i)"
  assumes hM_sep: "geotop_separates_pts UNIV geotop_euclidean_topology
...
proof -
  (** (1) M_i's end-points lie in Int B - S. Each M_i has two endpoints; they fall onto
         Int B (broken line), so pairs of endpoints define 2-point cuts of B. **)
  have h_endpoints_on_B:
    "\<forall>i<n. \<exists>E. card E = 2 \<and> E \<subseteq> Ms i \<and>
         (\<forall>e\<in>E. e \<in> geotop_arc_interior B {P, Q} - {S})" sorry
  (** (2) Since M separates R from S in \<bar>I\<close>, by the Kuratowski component argument one of
         the M_i's must contain a separating sub-broken-line, and its endpoints cut J into
         two arcs containing R and S respectively. **)
...
theorem Theorem_GT_9_2:
  ...
  assumes hJ: "geotop_is_n_sphere J (subspace_topology UNIV geotop_euclidean_topology J) 1"
  assumes hI: "I = geotop_polygon_interior J"
  assumes hA: "geotop_is_arc A (subspace_topology UNIV geotop_euclidean_topology A)"
  assumes hAJ: "A \<subseteq> J"
  shows "\<exists>v v' E. geotop_arc_endpoints A E \<and> v \<in> geotop_arc_interior A E
          \<and> geotop_segment v v' - {v} \<subseteq> I"
proof -
  (** (1) Pick any interior point v of A. There is a small disk D(v, r) around v in R^2
         such that D(v, r) \<setminus> {v} intersects I (the interior I is open and accumulates
         near v). **)
  obtain v where hv:
...
proof -
  (** (1) At level i, cover J by finitely many arcs g_k of diameter < 1/i (compactness
         of J + finite subcover of 1/(2i)-balls centred on J). **)
  have h_level_cover:
...
theorem Theorem_GT_9_4:
  ...
  assumes hJ: "geotop_is_n_sphere J (subspace_topology UNIV geotop_euclidean_topology J) 1"
  assumes hI: "I = geotop_polygon_interior J"
  assumes hG: "\<forall>i. finite (G i) \<and> (\<forall>g\<in>G i. geotop_is_arc g (subspace_topology UNIV geotop_euclidean_topology g) \<and> g \<subseteq> J)"
...
proof -
  (** (1) For each arc-endpoint v of G_i, Theorem 9.2 provides a linear-access interval
         vv' with vv' - v \<subseteq> I. Shrink v' to ensure disjoint H_i intervals. **)
  have h_per_vertex:
    "\<forall>i. \<exists>Hi. (\<forall>g\<in>G i. \<forall>E. geotop_arc_endpoints g E \<longrightarrow>
                (\<forall>v\<in>E. \<exists>!v'. (v, v') \<in> Hi))" sorry
  (** (2) Nested compatibility: at level i + 1, re-use v's interval if v is also an
         endpoint at level i + 1, but shrink its length if needed to fit inside the
         level-i interval. Handle new vertices (not in G_i) by fresh short intervals. **)
  have h_nested:
...
theorem Theorem_GT_9_5:
  ...
  assumes hA: "geotop_is_arc A (subspace_topology UNIV geotop_euclidean_topology A)"
  assumes hAJ: "A \<subseteq> J"
  assumes hv0: "geotop_segment v0 v0' - {v0} \<subseteq> I"
  assumes hv1: "geotop_segment v1 v1' - {v1} \<subseteq> I"
...
proof -
  (** (1) The \<epsilon>-tube T = N(A, \<epsilon>) \<inter> I is an open connected neighbourhood of the interior
         of A in I. Shrinking \<epsilon> if needed, the tube misses J \<setminus> A. **)
  have h_tube:
    "\<exists>T. T \<in> geotop_euclidean_topology \<and> T \<subseteq> I \<and>
         T \<subseteq> geotop_nbhd_set UNIV (\<lambda>x y. norm (x - y)) A \<epsilon> \<and>
         top1_connected_on T (subspace_topology UNIV geotop_euclidean_topology T)" sorry
...
theorem Theorem_GT_22_1:
  ...
  assumes hM_mfd: "geotop_n_manifold_with_boundary_on M (\<lambda>x y. norm (x - y)) 2"
  assumes hM_closed: "geotop_manifold_boundary M (\<lambda>x y. norm (x - y)) = {}"
  assumes hK: "geotop_is_complex K" and hK_poly: "geotop_polyhedron K = M"
  shows "\<exists>\<C>. geotop_is_open_cell_complex \<C> \<and>
             \<Union>\<C> = M \<and>
             geotop_open_cell_num_faces \<C> = 1"
proof -
  (** (1) Build a maximal sequence \<sigma>_1, \<sigma>_2, ..., \<sigma>_n of distinct 2-faces of K such that
         for each i < n, \<sigma>_{i+1} shares an edge e_i with \<union>_{j \<le> i} \<sigma>_j (constructively by
         greedy selection, since K is finite). **)
  obtain \<sigma>s :: "'a set list" and es :: "'a set list" where hseq:
    "(\<forall>i < length \<sigma>s. (\<sigma>s ! i) \<in> K \<and> geotop_simplex_dim (\<sigma>s ! i) 2) \<and>
     distinct \<sigma>s \<and>
     (\<forall>i. 0 < i \<and> i < length \<sigma>s \<longrightarrow>
          (\<exists>e. e \<in> K \<and> geotop_simplex_dim e 1 \<and> e = es ! (i - 1) \<and>
               e \<subseteq> (\<sigma>s ! i) \<and> e \<subseteq> \<Union>{\<sigma>s ! j | j. j < i}))" sorry
  (** (2) Maximality: if \<sigma>'\<in>K is a 2-face not in the sequence, then \<sigma>' has no edge in
         common with \<union>_i \<sigma>_i. **)
  have h_maximal:
    "\<forall>\<sigma>'\<in>K. geotop_simplex_dim \<sigma>' 2 \<and> \<sigma>' \<notin> set \<sigma>s \<longrightarrow>
       (\<forall>e\<in>K. geotop_simplex_dim e 1 \<and> e \<subseteq> \<sigma>' \<longrightarrow>
          \<not> (\<exists>\<tau>\<in>K. geotop_simplex_dim \<tau> 2 \<and> \<tau> \<in> set \<sigma>s \<and> e \<subseteq> \<tau>))" sorry
  (** (3) \<union>_i \<sigma>_i contains all or none of each open-star Int|St v|. Since M is connected,
         the union equals M. **)
  have h_cover:
    "\<Union>(set \<sigma>s) = M" sorry
  (** (4) Form the open cell-decomposition \<C> = {all vertices, all edges, (M - edges)}:
         a single open 2-face (M minus the 1-skeleton), all original edges of K. **)
  have h_cell_decomp:
...
theorem Theorem_GT_22_2:
  ...
  assumes hK: "geotop_is_complex K" and hL: "geotop_is_complex L" and hLK: "L \<subseteq> K"
  assumes hL_conn: "top1_connected_on (geotop_polyhedron L)
             (subspace_topology UNIV geotop_euclidean_topology (geotop_polyhedron L))"
  assumes hL_acyclic: "geotop_is_acyclic_linear_graph (geotop_polyhedron L)"
  shows "geotop_is_n_cell (geotop_regular_neighborhood K L)
           (subspace_topology UNIV geotop_euclidean_topology
              (geotop_regular_neighborhood K L)) 2"
proof -
  (** (1) Induct on the number of edges of L. Base case: L is a single edge of K. Then
         N(L) is the regular neighbourhood of a closed edge, which is a polyhedral 2-cell
         (a thickening of the edge in b^2 K). **)
  have h_base:
    "\<forall>L. geotop_is_complex L \<and> L \<subseteq> K \<and> geotop_is_acyclic_linear_graph (geotop_polyhedron L) \<and>
         card {\<sigma>\<in>L. geotop_simplex_dim \<sigma> 1} = 1 \<longrightarrow>
         geotop_is_n_cell (geotop_regular_neighborhood K L)
           (subspace_topology UNIV geotop_euclidean_topology
              (geotop_regular_neighborhood K L)) 2" sorry
  (** (2) Inductive step: given L connected acyclic with \<ge> 2 edges, pick any edge e of L.
         Then L - e is a disjoint union L_1 \<cup> L_2 of two connected acyclic subcomplexes
         (since L is a tree, removing an edge disconnects it into two subtrees). **)
  have h_split:
    "\<forall>L. geotop_is_complex L \<and> L \<subseteq> K \<and>
         top1_connected_on (geotop_polyhedron L)
           (subspace_topology UNIV geotop_euclidean_topology (geotop_polyhedron L)) \<and>
         geotop_is_acyclic_linear_graph (geotop_polyhedron L) \<and>
         card {\<sigma>\<in>L. geotop_simplex_dim \<sigma> 1} \<ge> 2 \<longrightarrow>
         (\<exists>e L\<^sub>1 L\<^sub>2. e \<in> L \<and> geotop_simplex_dim e 1 \<and>
                    L - {e} = L\<^sub>1 \<union> L\<^sub>2 \<and> L\<^sub>1 \<inter> L\<^sub>2 = {} \<and>
                    top1_connected_on (geotop_polyhedron L\<^sub>1)
                      (subspace_topology UNIV geotop_euclidean_topology (geotop_polyhedron L\<^sub>1)) \<and>
                    top1_connected_on (geotop_polyhedron L\<^sub>2)
                      (subspace_topology UNIV geotop_euclidean_topology (geotop_polyhedron L\<^sub>2)))"
    sorry
...
proof -
  (** (1) Triangulate M (Rado, Theorem 8.3) to obtain a finite triangulation K. **)
  obtain K where hK:
    "geotop_is_complex K \<and> geotop_polyhedron K = M" sorry
  (** (2) By Theorem 22.1, M has an open cell-decomposition \<C> with exactly one 2-face
         C^2 and edges from K. Let L be the 1-skeleton of \<C> (acyclic subgraph spanning all
         vertices: a spanning tree T + co-tree edges contributing the loops). **)
  have h_tree_cotree:
    "\<exists>\<C> T. geotop_is_open_cell_complex \<C> \<and> \<Union>\<C> = M \<and>
           geotop_open_cell_num_faces \<C> = 1 \<and>
           geotop_is_complex T \<and> T \<subseteq> K \<and>
           top1_connected_on (geotop_polyhedron T)
             (subspace_topology UNIV geotop_euclidean_topology (geotop_polyhedron T)) \<and>
           geotop_is_acyclic_linear_graph (geotop_polyhedron T)" sorry
  (** (3) Let T be the spanning tree, T' = L - T its complement; by Theorem 22.2, D =
         N(T) and D' = N(T') are 2-cells (regular neighbourhoods of disjoint acyclic
         graphs). Each loop edge in T' contributes a "strip" S_i = N(edge_i) which is
         also a 2-cell meeting D and D' each in a pair of disjoint arcs. **)
  have h_strips:
    "\<exists>D D' Ss. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2 \<and>
               geotop_is_n_cell D' (subspace_topology UNIV geotop_euclidean_topology D') 2 \<and>
               finite Ss \<and>
               (\<forall>S\<in>Ss. geotop_is_n_cell S (subspace_topology UNIV geotop_euclidean_topology S) 2) \<and>
  ...
  shows "\<exists>h m. 0 \<le> m \<and> m \<le> 2 \<and>
               (\<exists>M' f. geotop_is_sphere_with_n_handles h M' \<and>
                     ...
                     top1_homeomorphism_on M (subspace_topology UNIV geotop_euclidean_topology M)
                       M' (subspace_topology UNIV geotop_euclidean_topology M') f)"
proof -
  (** (1) Apply Theorem 22.3: M = D \<cup> D' \<cup> \<union>_i S_i, D, D' are 2-cells, each S_i is a
         strip 2-cell meeting D, D' in a pair of disjoint arcs. Cut M along each S_i and
         record whether the gluing reverses orientation (the strip S_i attaches as a
         handle iff it glues "parallel" through D, D'; as a Mobius band iff "anti-parallel"). **)
  obtain D D' Ss where h_decomp:
    "geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2 \<and>
     geotop_is_n_cell D' (subspace_topology UNIV geotop_euclidean_topology D') 2 \<and>
     finite Ss \<and>
     (\<forall>S\<in>Ss. geotop_is_n_cell S (subspace_topology UNIV geotop_euclidean_topology S) 2) \<and>
     M = D \<union> D' \<union> \<Union>Ss" sorry
  (** (2) Classify strips: let h = #handles, m_0 = #crosscaps (Mobius attachments).
         Canonical surgery moves: a pair of orientation-reversing strips can be converted
         into one handle + one crosscap; a handle + crosscap equals 3 crosscaps (Dyck's
         theorem). Hence the total (h, m) can be reduced to a normal form with 0 \<le> m \<le> 2. **)
  have h_normalize:
    "\<exists>h m. 0 \<le> m \<and> m \<le> 2 \<and>
           (\<exists>M' f. geotop_is_sphere_with_n_handles h M' \<and>
                  geotop_is_sphere_with_n_crosscaps m M' \<and>
                  top1_homeomorphism_on M (subspace_topology UNIV geotop_euclidean_topology M)
                    M' (subspace_topology UNIV geotop_euclidean_topology M') f)" sorry
...
proof -
  (** (1) For m = 0 (orientable): a 2-sphere with h handles has homology H_1 \<cong> Z^{2h}
         (each handle contributes a Z from its meridian and a Z from its longitude, with
         no torsion). Hence p^1 = 2h. **)
  have h_m0:
    "m = 0 \<longrightarrow> geotop_first_betti_number M = 2 * h" sorry
  (** (2) For m = 1 (projective plane + handles): H_1 has free rank 2h and one torsion
         factor Z_2 (from the crosscap). Free rank = p^1 = 2h. **)
  have h_m1:
    "m = 1 \<longrightarrow> geotop_first_betti_number M = 2 * h" sorry
  (** (3) For m = 2 (Klein-bottle-like + handles): H_1 has free rank 2h + 1 and one
...
proof -
  (** (1) By Theorem 22_4, M \<cong> sphere with h handles and m crosscaps, 0 \<le> m \<le> 2. **)
  obtain h m where h_classify:
    "0 \<le> m \<and> m \<le> 2 \<and>
     (\<exists>M' f. geotop_is_sphere_with_n_handles h M' \<and>
            geotop_is_sphere_with_n_crosscaps m M' \<and>
            top1_homeomorphism_on M (subspace_topology UNIV geotop_euclidean_topology M)
              M' (subspace_topology UNIV geotop_euclidean_topology M') f)" sorry
  (** (2) Theorem 22.5: \<chi>(M) = 2 - (2h + m). **)
...
theorem Theorem_GT_22_8:
  ...
  assumes hM1_h: "geotop_is_sphere_with_n_handles h1 M1" and hM1_m: "geotop_is_sphere_with_n_crosscaps m1 M1"
  assumes hm1: "m1 \<le> 2"
  assumes hM2_h: "geotop_is_sphere_with_n_handles h2 M2" and hM2_m: "geotop_is_sphere_with_n_crosscaps m2 M2"
  assumes hm2: "m2 \<le> 2"
  shows "(\<exists>f. top1_homeomorphism_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)
         ...
         (h1 = h2 \<and> m1 = m2)"
proof -
  (** (1) (\<Leftarrow>) If h_1 = h_2 and m_1 = m_2, both M_i are spheres with the same handles /
         crosscaps; a canonical homeomorphism between them exists since any two such
         surfaces are homeomorphic (standard construction via attaching homeomorphisms). **)
  have h_suff:
    "h1 = h2 \<and> m1 = m2 \<longrightarrow>
       (\<exists>f. top1_homeomorphism_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)
             M2 (subspace_topology UNIV geotop_euclidean_topology M2) f)" sorry
  (** (2) (\<Rightarrow>) Homeomorphic M_1, M_2 have the same orientability, and \<chi>(M_i) is a
         topological invariant (Theorem 21_5). **)
  have h_inv:
    "(\<exists>f. top1_homeomorphism_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)
             M2 (subspace_topology UNIV geotop_euclidean_topology M2) f) \<longrightarrow>
       geotop_manifold_euler M1 = geotop_manifold_euler M2 \<and>
       (geotop_set_orientable M1 \<longleftrightarrow> geotop_set_orientable M2)" sorry
...
theorem Theorem_GT_22_9:
  ...
  assumes hM1_cpt: "top1_compact_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)"
  assumes hM1_conn: "top1_connected_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)"
  assumes hM1_mfd: "geotop_n_manifold_with_boundary_on M1 (\<lambda>x y. norm (x - y)) 2"
  assumes hM1_closed: "geotop_manifold_boundary M1 (\<lambda>x y. norm (x - y)) = {}"
  assumes hK1: "geotop_is_complex K1" and hK1_poly: "geotop_polyhedron K1 = M1"
  assumes hM2_cpt: "top1_compact_on M2 (subspace_topology UNIV geotop_euclidean_topology M2)"
  assumes hM2_conn: "top1_connected_on M2 (subspace_topology UNIV geotop_euclidean_topology M2)"
  assumes hM2_mfd: "geotop_n_manifold_with_boundary_on M2 (\<lambda>x y. norm (x - y)) 2"
  assumes hM2_closed: "geotop_manifold_boundary M2 (\<lambda>x y. norm (x - y)) = {}"
  assumes hK2: "geotop_is_complex K2" and hK2_poly: "geotop_polyhedron K2 = M2"
  shows "(\<exists>f. top1_homeomorphism_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)
                M2 (subspace_topology UNIV geotop_euclidean_topology M2) f) \<longleftrightarrow>
         ((geotop_is_orientable K1 \<longleftrightarrow> geotop_is_orientable K2) \<and>
          geotop_manifold_euler M1 = geotop_manifold_euler M2)"
proof -
  (** (1) By Theorem 22_4, each M_i is homeomorphic to a sphere with h_i handles and
         m_i crosscaps, 0 \<le> m_i \<le> 2. **)
  obtain h1 m1 h2 m2 where h_classify:
    "0 \<le> m1 \<and> m1 \<le> 2 \<and> 0 \<le> m2 \<and> m2 \<le> 2 \<and>
     (\<exists>M1' f1. geotop_is_sphere_with_n_handles h1 M1' \<and>
                geotop_is_sphere_with_n_crosscaps m1 M1' \<and>
                top1_homeomorphism_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)
                  M1' (subspace_topology UNIV geotop_euclidean_topology M1') f1) \<and>
     (\<exists>M2' f2. geotop_is_sphere_with_n_handles h2 M2' \<and>
                geotop_is_sphere_with_n_crosscaps m2 M2' \<and>
                top1_homeomorphism_on M2 (subspace_topology UNIV geotop_euclidean_topology M2)
                  M2' (subspace_topology UNIV geotop_euclidean_topology M2') f2)" sorry
...
theorem Theorem_GT_22_11:
  ...
  assumes hM_mfd: "geotop_n_manifold_with_boundary_on M (\<lambda>x y. norm (x - y)) 2"
  assumes hM_closed: "geotop_manifold_boundary M (\<lambda>x y. norm (x - y)) = {}"
  assumes hM_simp_conn: "\<forall>P\<in>M. geotop_simply_connected M
...
proof -
  (** (1) By Theorem 22.4, M is homeomorphic to a sphere with h handles and m crosscaps,
         0 \<le> m \<le> 2. Fix such a classification. **)
  obtain h m where h_classify:
    "0 \<le> m \<and> m \<le> 2 \<and>
     (\<exists>M' f. geotop_is_sphere_with_n_handles h M' \<and>
            geotop_is_sphere_with_n_crosscaps m M' \<and>
            top1_homeomorphism_on M (subspace_topology UNIV geotop_euclidean_topology M)
              M' (subspace_topology UNIV geotop_euclidean_topology M') f)" sorry
  (** (2) Fundamental group of such M' is free of rank 2h + m (generators: 2h from
...
theorem Theorem_GT_24_1:
  ...
  assumes hcov: "geotop_is_covering Mt T\<^sub>M M T\<^sub>M' g"
  assumes h\<Delta>: "geotop_is_n_cell \<Delta> T\<^sub>\<Delta> 2"
  assumes hf_cont: "top1_continuous_map_on \<Delta> T\<^sub>\<Delta> M T\<^sub>M' f"
  fixes Q\<^sub>0 :: 'c and Pt\<^sub>0 :: 'a
  assumes hQ\<^sub>0: "Q\<^sub>0 \<in> \<Delta>" and hPt\<^sub>0: "Pt\<^sub>0 \<in> Mt" and hmatch: "g Pt\<^sub>0 = f Q\<^sub>0"
  shows "\<exists>!ft. geotop_is_lifting Mt T\<^sub>M M T\<^sub>M' g \<Delta> T\<^sub>\<Delta> f ft \<and> ft Q\<^sub>0 = Pt\<^sub>0"
proof -
  (** (1) Cover \<Delta> (compact 2-cell) by finitely many open neighbourhoods {V_j} each of
         which has f-image contained in an evenly covered open set U_j of M (Lebesgue
         number of the cover {f^{-1}(U)} for U evenly covered). **)
  obtain Vs :: "'c set set" and Us :: "'b set set" where h_good_cover:
...
theorem Theorem_GT_24_2:
  ...
  fixes g :: "'a \<Rightarrow> 'b"
  assumes hcov: "geotop_is_covering Mt T\<^sub>M M T\<^sub>M' g"
  assumes hPt\<^sub>0: "Pt\<^sub>0 \<in> Mt" and hgPt\<^sub>0: "g Pt\<^sub>0 \<in> M"
...
proof -
  (** (1) Define g_0*: \<pi>(Mt, Pt_0) \<to> \<pi>(M, P_0) by g_0*([pt]) = [g \<circ> pt]; well-defined
         by functoriality of Theorem 14.5. **)
  have h_induced:
  ...
  (** (2) Injectivity: suppose [g \<circ> pt_1] = [g \<circ> pt_2]. Take a homotopy F: [0,1]^2 \<to> M
         between g \<circ> pt_1 and g \<circ> pt_2. By Theorem 24.1 (path lifting version), lift F to
...
theorem Theorem_GT_24_3:
  ...
  assumes hcov: "geotop_is_covering Mt T\<^sub>M M T\<^sub>M' g"
  assumes hPt\<^sub>0: "Pt\<^sub>0 \<in> Mt"
  assumes hp_cp: "geotop_closed_path_on M T\<^sub>M' (g Pt\<^sub>0) p"
...
proof -
  (** (1) (\<Rightarrow>) If [p] \<in> image of g_0*, there is pt_1 \<in> CP(Mt, Pt_0) with [g \<circ> pt_1] = [p]
         in \<pi>(M, P_0). By Theorem 24.1 (path homotopy lifting), lift the homotopy from
         g \<circ> pt_1 to p starting at Pt_0 to get a homotopy in Mt from pt_1 to some lift pt
         of p with pt(0) = Pt_0. Closedness of pt follows from pt(1) = Pt_0 (lift of the
         homotopy endpoints). **)
  have h_forward:
    "(\<exists>g\<^sub>0. (\<forall>C\<in>geotop_pi Mt T\<^sub>M Pt\<^sub>0. g\<^sub>0 C \<in> geotop_pi M T\<^sub>M' (g Pt\<^sub>0)) \<and>
              geotop_pi_class M T\<^sub>M' (g Pt\<^sub>0) p \<in> g\<^sub>0 ` geotop_pi Mt T\<^sub>M Pt\<^sub>0) \<longrightarrow>
     (\<exists>pt. geotop_closed_path_on Mt T\<^sub>M Pt\<^sub>0 pt \<and> (\<forall>t. g (pt t) = p t))" sorry
  (** (2) (\<Leftarrow>) If p lifts to pt \<in> CP(Mt, Pt_0), then [p] = [g \<circ> pt] = g_0*([pt]) \<in> image
         of g_0*. **)
  have h_backward:
...
theorem Theorem_GT_24_4:
  ...
  fixes M :: "'b set" and T\<^sub>M' :: "'b set set"
  fixes g :: "'a \<Rightarrow> 'b"
  assumes hkcov: "geotop_is_k_fold_covering k Mt T\<^sub>M M T\<^sub>M' g"
  assumes hPt\<^sub>0: "Pt\<^sub>0 \<in> Mt"
  shows "\<exists>cosets. card cosets = k \<and>
                  \<Union>cosets = geotop_pi M T\<^sub>M' (g Pt\<^sub>0)"
proof -
  (** (1) By Theorem 24.2, \<pi>_0 = g_0*(\<pi>(Mt, Pt_0)) is a subgroup of \<pi>(M, P_0). Partition
         \<pi>(M, P_0) into left cosets of \<pi>_0. **)
  have h_cosets:
    "\<exists>g\<^sub>0 cosets. (\<forall>C\<in>cosets. C \<subseteq> geotop_pi M T\<^sub>M' (g Pt\<^sub>0)) \<and>
                 \<Union>cosets = geotop_pi M T\<^sub>M' (g Pt\<^sub>0) \<and>
                 (\<forall>c\<^sub>1\<in>cosets. \<forall>c\<^sub>2\<in>cosets. c\<^sub>1 \<noteq> c\<^sub>2 \<longrightarrow> c\<^sub>1 \<inter> c\<^sub>2 = {})" sorry
  (** (2) Correspondence: each coset [p] \<pi>_0 corresponds to an endpoint of the lifted
         path starting at Pt_0 (Theorem 24.1 path lifting). Different cosets give
         different endpoints, same coset gives same endpoint. Hence #cosets = #{Q \<in> Mt |
         g(Q) = P_0} = k. **)
  have h_card_k:
...
theorem Theorem_GT_24_5:
  ...
  assumes hM_poly: "(\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = M)"
  assumes hM_conn: "top1_connected_on M (subspace_topology UNIV geotop_euclidean_topology M)"
  assumes hP\<^sub>0: "P\<^sub>0 \<in> M"
  assumes hindexk: "\<exists>\<pi> cosets. \<pi> \<subseteq> geotop_pi M (subspace_topology UNIV geotop_euclidean_topology M) P\<^sub>0
                     \<and> card cosets = k
...
proof -
  (** (1) Construct Mt as pairs (Q, coset) where Q \<in> M and coset is a left coset of the
         subgroup \<pi> in \<pi>(M, P_0) (chosen via a path-lift equivalence). **)
  obtain \<pi> cosets where h_subgroup:
    "\<pi> \<subseteq> geotop_pi M (subspace_topology UNIV geotop_euclidean_topology M) P\<^sub>0 \<and>
     card cosets = k \<and>
...
theorem Theorem_GT_24_6:
  ...
  assumes hcov: "geotop_is_covering Mt (subspace_topology UNIV geotop_euclidean_topology Mt)
             M (subspace_topology UNIV geotop_euclidean_topology M) g"
  assumes hK: "geotop_is_complex K" and hK_poly: "geotop_polyhedron K = M"
  shows "\<exists>Kt. geotop_is_complex Kt \<and> geotop_polyhedron Kt = Mt \<and>
              (\<forall>sigmat\<in>Kt. \<exists>\<sigma>\<in>K. top1_homeomorphism_on sigmat
                  ...
                  (\<lambda>P. g P))"
proof -
  (** (1) Subdivide K so each simplex lies in some evenly covered open set of M (finite
         Lebesgue number argument). **)
  have h_subdivide:
    "\<exists>K\<^sub>s. geotop_is_complex K\<^sub>s \<and> geotop_polyhedron K\<^sub>s = M \<and>
          (\<forall>\<sigma>\<in>K\<^sub>s. \<exists>U\<in>(subspace_topology UNIV geotop_euclidean_topology M). \<sigma> \<subseteq> U)" sorry
  (** (2) For each simplex \<sigma> \<in> K_s and each sheet of g over \<sigma>, get a lifted simplex
         \<sigma>~ \<subseteq> Mt homeomorphic to \<sigma>. Collect all such \<sigma>~ into Kt. **)
  obtain Kt where h_lift_simplexes:
    "geotop_is_complex Kt \<and> geotop_polyhedron Kt = Mt \<and>
     (\<forall>sigmat\<in>Kt. \<exists>\<sigma>\<in>K. top1_homeomorphism_on sigmat
           (subspace_topology UNIV geotop_euclidean_topology sigmat)
           \<sigma> (subspace_topology UNIV geotop_euclidean_topology \<sigma>)
           (\<lambda>P. g P))" sorry
...
theorem Theorem_GT_24_7:
  ...
  assumes hM_3mfd: "geotop_n_manifold_with_boundary_on M (\<lambda>x y. norm (x - y)) 3"
  assumes hM_conn: "top1_connected_on M (subspace_topology UNIV geotop_euclidean_topology M)"
  assumes hM_nonorient: "\<not> geotop_is_orientable_3_manifold K"
  shows "\<exists>Mt (g::'a \<Rightarrow> 'a). geotop_is_k_fold_covering 2 Mt
           (subspace_topology UNIV geotop_euclidean_topology Mt)
           M (subspace_topology UNIV geotop_euclidean_topology M) g"
proof -
  (** (1) Assign to each 3-simplex \<sigma>^3 \<in> K an orientation \<omega>(\<sigma>^3) \<in> {+1, -1} (arbitrarily
...
theorem Theorem_GT_24_8:
  ...
  assumes hM_conn: "top1_connected_on M (subspace_topology UNIV geotop_euclidean_topology M)"
  assumes hK_orient: "geotop_is_orientable_3_manifold K"
  assumes hBd_not_sphere: "\<exists>C. (\<exists>P\<in>geotop_manifold_boundary M (\<lambda>x y. norm (x - y)).
                  C = geotop_component_at UNIV geotop_euclidean_topology
  ...
  shows "\<exists>Mt (g::'a \<Rightarrow> 'a). geotop_is_k_fold_covering 2 Mt
           ...
           M (subspace_topology UNIV geotop_euclidean_topology M) g"
proof -
  (** (1) Let C be the non-sphere boundary component; by Theorem 22_4 it is a sphere with
...
theorem Theorem_GT_24_9:
  ...
  assumes hS_poly: "(\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = S)"
  assumes hS_in_K: "S \<subseteq> geotop_polyhedron K"
...
proof -
  (** (1) (=>) Given S is a CST via decomposition into n consecutive 3-cells C_i with
         cyclic 2-cell gluings. Walk around the cycle: parametrise t in [0, 1] with
         sub-intervals [(i-1)/n, i/n] mapping to C_i, and the common 2-cell D = C_0 \<inter> C_n. **)
  have h_forward:
    "geotop_is_CST S \<longrightarrow>
       (\<exists>(\<sigma>2::(real^2) set) (\<phi>::(real^2) \<times> real \<Rightarrow> 'a).
          geotop_simplex_dim \<sigma>2 2 \<and>
          \<phi> ` (\<sigma>2 \<times> {t. 0 \<le> t \<and> t \<le> 1}) = S \<and>
          (\<exists>D. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2 \<and>
               \<phi> ` (\<sigma>2 \<times> {0}) = D \<and> \<phi> ` (\<sigma>2 \<times> {1}) = D))" sorry
  (** (2) (<=) Given the parametrisation \<phi>: \<sigma>^2 \<times> [0, 1] \<to> S identifying top and bottom
         faces to one 2-cell D, partition [0, 1] at t_i = i/n into n parts to get C_i =
...
theorem Theorem_GT_24_10:
  ...
  assumes hS1: "geotop_is_CST S1" and hS2: "geotop_is_CST S2"
...
proof -
  (** (1) By Theorem 24.9, each S_i has a standard parametrisation \<phi>_i: \<sigma>^2 \<times> [0, 1] \<to>
         S_i with top and bottom 2-faces identified to one 2-cell. **)
  obtain \<phi>1 \<phi>2 \<sigma>2 where h_std_params:
    "geotop_simplex_dim \<sigma>2 2 \<and>
     \<phi>1 ` (\<sigma>2 \<times> {t::real. 0 \<le> t \<and> t \<le> 1}) = S1 \<and>
...
theorem Theorem_GT_24_11:
  ...
  assumes hK_orient: "geotop_is_orientable_3_manifold K"
  assumes hJ: "geotop_is_polygon J" and hJ_in_K: "J \<subseteq> geotop_polyhedron K"
  assumes hJ_subcomplex: "\<exists>K'. geotop_is_complex K' \<and> geotop_is_subdivision K K' \<and>
...
proof -
  (** (1) Subdivide K so J is a subcomplex L. Take S = N(L) = regular neighbourhood in
...
theorem Theorem_GT_24_12:
  ...
  assumes hK: "geotop_is_complex K"
  assumes hK_3mfd: "geotop_n_manifold_with_boundary_on (geotop_polyhedron K) (\<lambda>x y. norm (x - y)) 3"
  assumes hJ: "geotop_is_polygon J" and hJ_sub: "J \<subseteq> geotop_polyhedron K"
  assumes hP\<^sub>0: "P\<^sub>0 \<in> J"
  assumes hJ_contractible: "\<exists>p. geotop_closed_path_on (geotop_polyhedron K)
                (subspace_topology UNIV geotop_euclidean_topology (geotop_polyhedron K))
                P\<^sub>0 p \<and>
                p ` {0..1} = J \<and>
...
proof -
  (** (1) J is contractible; take a PL null-homotopy f: D^2 \<to> |K| with f | S^1 = J. **)
  obtain f :: "real \<times> real \<Rightarrow> 'a" where h_null:
    "(\<forall>t y. 0 \<le> t \<and> t \<le> 1 \<and> 0 \<le> y \<and> y \<le> 1 \<longrightarrow> f (t, y) \<in> geotop_polyhedron K) \<and>
     (\<forall>t. 0 \<le> t \<and> t \<le> 1 \<longrightarrow> f (t, 0) \<in> J) \<and>
     (\<forall>t. 0 \<le> t \<and> t \<le> 1 \<longrightarrow> f (t, 1) = P\<^sub>0)" sorry
  (** (2) Subdivide K so J is a subcomplex L. N(L) inherits the CST structure from the
         argument of Theorem 24.11 locally around the disc swept by f, which thickens J
         into an untwisted solid torus. **)
  obtain K' L where hKL:
    "geotop_is_complex K' \<and> geotop_is_subdivision K K' \<and>
     L \<subseteq> K' \<and> geotop_polyhedron L = J" sorry
  (** (3) The combinatorial cyclic structure of N(L) is obtained by slicing along the
         edges of L with each C_i a 3-cell thickening of an edge. The contractibility of
...
theorem Theorem_GT_26_1:
  ...
  assumes hK: "geotop_is_complex K" and hM3: "M3 = geotop_polyhedron K"
  assumes hM3_mfd: "geotop_n_manifold_with_boundary_on M3 (\<lambda>x y. norm (x - y)) 3"
  assumes hM2_mfd: "geotop_n_manifold_with_boundary_on M2 (\<lambda>x y. norm (x - y)) 2"
  assumes hM2_int: "M2 \<subseteq> geotop_manifold_interior M3 (\<lambda>x y. norm (x - y))"
  assumes hN_mfd: "geotop_n_manifold_with_boundary_on N (\<lambda>x y. norm (x - y)) 3"
  assumes hN_in_M3: "N \<subseteq> M3"
  assumes hM2_as_Bd: "\<exists>Cs. (\<forall>C\<in>Cs. C \<subseteq> geotop_manifold_boundary N (\<lambda>x y. norm (x - y))) \<and>
                M2 = \<Union>Cs"
  shows "geotop_is_two_sided M2 M3"
proof -
  (** (1) Take a small open neighbourhood V of M^2 in M^3 contained inside N \<union> (M^3 - N);
         V \<inter> Int N and V \<inter> (M^3 - Cl N) are each connected (as collar/exterior neighbourhoods
         of M^2). **)
  obtain V where hV:
    "V \<in> geotop_euclidean_topology \<and> M2 \<subseteq> V \<and> V \<subseteq> M3" sorry
  (** (2) For every sufficiently small open W with M^2 \<subseteq> W \<subseteq> V, W - M^2 has at least two
         components (one inside N, one outside), since M^2 is in \<partial>N and separates the
         neighbourhood. **)
  have h_separates:
    "\<forall>W\<in>geotop_euclidean_topology. M2 \<subseteq> W \<and> W \<subseteq> V \<and>
       top1_connected_on W (subspace_topology UNIV geotop_euclidean_topology W) \<longrightarrow>
       \<not> top1_connected_on (W - M2)
...
theorem Theorem_GT_26_2:
  ...
  assumes hK: "geotop_is_complex K" and hM3: "M3 = geotop_polyhedron K"
  assumes hM3_mfd: "geotop_n_manifold_with_boundary_on M3 (\<lambda>x y. norm (x - y)) 3"
  assumes hBd_cpt: "top1_compact_on (geotop_manifold_boundary M3 (\<lambda>x y. norm (x - y)))
             (subspace_topology UNIV geotop_euclidean_topology
...
proof -
  (** (1) Cover Bd M^3 with finitely many half-simplex neighbourhoods (Theorem 23_2, 23_4):
         Bd M^3 \<subseteq> \<union>_i f_i(\<sigma>_i^2) with f_i: \<sigma>_i^3 \<leftrightarrow> C_i^3 local charts. **)
  obtain Cs :: "(real^3) set set" and fs where h_cover:
    "finite Cs \<and>
     (\<forall>C\<in>Cs. \<exists>f. \<exists>\<sigma>3 \<sigma>2::(real^3) set. geotop_simplex_dim \<sigma>3 3 \<and>
              top1_homeomorphism_on \<sigma>3
                 (subspace_topology UNIV geotop_euclidean_topology \<sigma>3) C
                 (subspace_topology UNIV geotop_euclidean_topology C) f)" sorry
...
theorem Theorem_GT_26_3:
  ...
  assumes hM2_mfd: "geotop_n_manifold_with_boundary_on M2 (\<lambda>x y. norm (x - y)) 2"
  assumes hM2_int: "M2 \<subseteq> geotop_manifold_interior M3 (\<lambda>x y. norm (x - y))"
  assumes hM2_twosided: "geotop_is_two_sided M2 (geotop_manifold_interior M3 (\<lambda>x y. norm (x - y)))"
  shows "\<exists>W. geotop_is_bicollar_neighborhood W M2 M3"
proof -
  (** (1) M^2 two-sided means a connected neighbourhood V of M^2 in Int M^3 is separated
         by M^2 into two sides V \<cap> (sides): "V^+" and "V^-". **)
  obtain V Vp Vm where h_two_sides:
    "V \<in> geotop_euclidean_topology \<and> M2 \<subseteq> V \<and>
     V \<subseteq> geotop_manifold_interior M3 (\<lambda>x y. norm (x - y)) \<and>
     V - M2 = Vp \<union> Vm \<and> Vp \<inter> Vm = {}" sorry
...
theorem Theorem_GT_26_4:
  ...
  assumes hM2_int: "M2 \<subseteq> geotop_manifold_interior M3 (\<lambda>x y. norm (x - y))"
  assumes hM2_twosided: "geotop_is_two_sided M2 (geotop_manifold_interior M3 (\<lambda>x y. norm (x - y)))"
  assumes hker_nontriv: "\<exists>P\<^sub>0\<in>M2. \<exists>p. geotop_closed_path_on M2
  ...
  shows "\<exists>\<Delta> C. geotop_is_n_cell \<Delta>
             ...
             (\<exists>P\<^sub>0\<in>C. \<exists>p.
                ...
                \<not> geotop_path_equiv M2 (subspace_topology UNIV geotop_euclidean_topology M2)
                     P\<^sub>0 p (\<lambda>t. P\<^sub>0))"
proof -
  (** (1) By Theorem 26_3, M^2 has a bicollar neighbourhood W = M^2 \<times> [-1, 1] in Int M^3.
         Cut M^3 open along M^2 to form a new 3-manifold M^3_cut with M^2 (two copies)
         as boundary components, via the bicollar. **)
  obtain M\<^sub>3\<^sub>c W where h_cut:
    "geotop_is_bicollar_neighborhood W M2 M3 \<and>
     geotop_n_manifold_with_boundary_on M\<^sub>3\<^sub>c (\<lambda>x y. norm (x - y)) 3 \<and>
     M2 \<subseteq> geotop_manifold_boundary M\<^sub>3\<^sub>c (\<lambda>x y. norm (x - y))" sorry
  (** (2) A loop L in M^2 which is contractible in M^3 but not in M^2 maps into one
         boundary copy of M^2 in M^3_cut and is still contractible there. Apply the Loop
         theorem (Theorem 25_2) to M^3_cut with boundary component B = M^2 to obtain a
         polyhedral 2-cell \<Delta> with Bd \<Delta> non-contractible in M^2. **)
  obtain \<Delta> C where h_loop_thm_applied:
...
theorem Theorem_GT_26_5:
  ...
  assumes hK: "geotop_is_complex K" and hM3: "M3 = geotop_polyhedron K"
  assumes hM3_mfd: "geotop_n_manifold_with_boundary_on M3 (\<lambda>x y. norm (x - y)) 3"
  assumes hM2_closed: "closedin_on M3 (subspace_topology UNIV geotop_euclidean_topology M3) M2"
  assumes hM2_mfd: "geotop_n_manifold_with_boundary_on M2 (\<lambda>x y. norm (x - y)) 2"
  assumes hM2_int: "M2 \<subseteq> geotop_manifold_interior M3 (\<lambda>x y. norm (x - y))"
  assumes hM2_twosided: "geotop_is_two_sided M2 (geotop_manifold_interior M3 (\<lambda>x y. norm (x - y)))"
  assumes hker_nontriv: "\<exists>P\<^sub>0\<in>M2. \<exists>p. geotop_closed_path_on M2
  ...
  shows "\<exists>\<Delta> C. geotop_is_n_cell \<Delta>
             ...
             C = \<Delta> \<inter> M2"
proof -
  (** (1) The closed (but not necessarily compact) M^2 can be exhausted by an increasing
         sequence of compact submanifolds M^2_n with M^2 = \<union>_n M^2_n. The kernel witness
         lies in some M^2_n (compactness of the PL null-homotopy image). **)
  obtain n M2\<^sub>n where h_compact_piece:
    "top1_compact_on M2\<^sub>n (subspace_topology UNIV geotop_euclidean_topology M2\<^sub>n) \<and>
     geotop_n_manifold_with_boundary_on M2\<^sub>n (\<lambda>x y. norm (x - y)) 2 \<and>
     M2\<^sub>n \<subseteq> M2" sorry
...
theorem Theorem_GT_26_6:
  ...
  assumes hM2_mfd: "geotop_n_manifold_with_boundary_on M2 (\<lambda>x y. norm (x - y)) 2"
  assumes hM2_closed: "geotop_manifold_boundary M2 (\<lambda>x y. norm (x - y)) = {}"
  assumes hM2_poly: "\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = M2"
...
proof -
  (** (1) A compact connected polyhedral 2-manifold in R^3 is orientable (Theorem 26_8).
         Orientable closed 2-manifold has a globally defined normal direction. **)
  have h_orientable:
    "\<exists>Km. geotop_is_complex Km \<and> geotop_polyhedron Km = M2 \<and>
...
theorem Theorem_GT_26_7:
  ...
  assumes hM1_conn: "top1_connected_on M1 (subspace_topology UNIV geotop_euclidean_topology M1)"
  assumes hM2_conn: "top1_connected_on M2 (subspace_topology UNIV geotop_euclidean_topology M2)"
  assumes hM3_conn: "top1_connected_on M3 (subspace_topology UNIV geotop_euclidean_topology M3)"
  assumes hM1_mfd: "geotop_n_manifold_with_boundary_on M1 (\<lambda>x y. norm (x - y)) 2"
  assumes hM2_mfd: "geotop_n_manifold_with_boundary_on M2 (\<lambda>x y. norm (x - y)) 2"
  assumes hM3_mfd: "geotop_n_manifold_with_boundary_on M3 (\<lambda>x y. norm (x - y)) 2"
  assumes hBd12: "geotop_manifold_boundary M1 (\<lambda>x y. norm (x - y))
           = geotop_manifold_boundary M2 (\<lambda>x y. norm (x - y))"
  assumes hBd23: "geotop_manifold_boundary M2 (\<lambda>x y. norm (x - y))
           = geotop_manifold_boundary M3 (\<lambda>x y. norm (x - y))"
  assumes hInt12: "geotop_manifold_interior M1 (\<lambda>x y. norm (x - y))
           \<inter> geotop_manifold_interior M2 (\<lambda>x y. norm (x - y)) = {}"
  assumes hInt23: "geotop_manifold_interior M2 (\<lambda>x y. norm (x - y))
           \<inter> geotop_manifold_interior M3 (\<lambda>x y. norm (x - y)) = {}"
  assumes hInt13: "geotop_manifold_interior M1 (\<lambda>x y. norm (x - y))
           \<inter> geotop_manifold_interior M3 (\<lambda>x y. norm (x - y)) = {}"
...
proof -
  ...
  have h_pair_boundary:
    "\<exists>(E::(real^3) set) Ma Mb. (Ma = M1 \<or> Ma = M2 \<or> Ma = M3) \<and>
                               (Mb = M1 \<or> Mb = M2 \<or> Mb = M3) \<and> Ma \<noteq> Mb \<and>
...
theorem Theorem_GT_27_1:
  ...
  assumes hN_in_M3: "N \<subseteq> M3"
  assumes hN_mfd: "geotop_n_manifold_with_boundary_on N (\<lambda>x y. norm (x - y)) 3"
  assumes hN_poly: "(\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = N)"
  assumes hM2_in_BdN: "M2 \<subseteq> geotop_manifold_boundary N (\<lambda>x y. norm (x - y))"
  assumes hM2_mfd: "geotop_n_manifold_with_boundary_on M2 (\<lambda>x y. norm (x - y)) 2"
  assumes hh_cellular: "geotop_is_cellular_PLH M2 h"
  assumes hW_open: "W \<in> geotop_euclidean_topology" and hM2_in_W: "M2 \<subseteq> W"
...
proof -
  (** (1) Extract the 2-cell d \<subseteq> M^2 where h is supported (from cellular-PLH definition):
         h = identity on M^2 - d. **)
  obtain d where hd:
    "geotop_is_n_cell d (subspace_topology UNIV geotop_euclidean_topology d) 2 \<and>
     (\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = d) \<and>
     d \<subseteq> M2 \<and> (\<forall>P\<in>M2 - d. h P = P)" sorry
  (** (2) Take a bicollar neighbourhood V of d in N \<union> (M^3 - N), contained inside W.
         Thicken d to a 3-cell D_3 = d \<times> [-\<epsilon>, \<epsilon>] via the bicollar (Theorem 26_3 or local
         half-simplex chart). **)
  obtain D3 where hD3:
    "geotop_is_n_cell D3 (subspace_topology UNIV geotop_euclidean_topology D3) 3 \<and>
     D3 \<subseteq> W \<and> d \<subseteq> D3 \<and>
     (\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = D3)" sorry
  (** (3) Extend h to a PLH h~: D_3 \<leftrightarrow> D_3 via cone-on-boundary: on d \<times> {-\<epsilon>, \<epsilon>} act by
         identity (collar); on d \<times> {0} apply h; interpolate linearly. h~ is supported
         inside D_3 \<subseteq> W. **)
  have h_extend_to_D3:
    "\<exists>h\<^sub>0. top1_homeomorphism_on UNIV geotop_euclidean_topology
            UNIV geotop_euclidean_topology h\<^sub>0 \<and>
          (\<forall>P\<in>M2. h\<^sub>0 P = h P) \<and>
...
theorem Theorem_GT_27_2:
  ...
  assumes hA: "geotop_is_k_annulus 1 A"
  assumes hJ: "geotop_is_polygon J" and hJ': "geotop_is_polygon J'"
  assumes hJ_in_A: "J \<subseteq> geotop_manifold_interior A (\<lambda>x y. norm (x - y))"
  assumes hJ'_in_A: "J' \<subseteq> geotop_manifold_interior A (\<lambda>x y. norm (x - y))"
  assumes hJ_no_cell: "\<not> (\<exists>D. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2
                  \<and> D \<subseteq> A \<and> geotop_frontier UNIV geotop_euclidean_topology D = J)"
  assumes hJ'_no_cell: "\<not> (\<exists>D. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2
                  \<and> D \<subseteq> A \<and> geotop_frontier UNIV geotop_euclidean_topology D = J')"
  shows "\<exists>h. top1_homeomorphism_on A (subspace_topology UNIV geotop_euclidean_topology A)
              ...
              (\<exists>hs. hs \<noteq> [] \<and>
                    ...
                    h = foldr (\<circ>) hs id)"
proof -
  (** (1) Since J, J' do not bound 2-cells in A, they are essential core polygons; each
         is isotopic to the "core circle" of the annulus (the central parallel). **)
  have h_essential_both:
    "\<exists>h\<^sub>J. top1_homeomorphism_on A (subspace_topology UNIV geotop_euclidean_topology A)
            A (subspace_topology UNIV geotop_euclidean_topology A) h\<^sub>J \<and>
          (\<exists>J\<^sub>0. h\<^sub>J ` J = J\<^sub>0) \<and>
          (\<forall>P\<in>geotop_manifold_boundary A (\<lambda>x y. norm (x - y)). h\<^sub>J P = P)" sorry
  (** (2) Break the isotopy into a finite sequence of cellular homeomorphisms supported
         on small 2-cells of A: slide J across one 2-cell at a time to reach J'. **)
  have h_cellular_chain:
...
proof -
  (** (1) If J bounds a 2-cell in A, done (first disjunct). **)
  have h_bounds_case:
...
theorem Theorem_GT_27_4:
  ...
  assumes hM3_mfd: "geotop_n_manifold_with_boundary_on M3 (\<lambda>x y. norm (x - y)) 3"
  assumes hM3_poly: "(\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = M3)"
  assumes hN_in_M3: "N \<subseteq> M3"
  assumes hN_mfd: "geotop_n_manifold_with_boundary_on N (\<lambda>x y. norm (x - y)) 3"
  assumes hN_poly: "(\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = N)"
  assumes hA_mfd: "geotop_n_manifold_with_boundary_on A (\<lambda>x y. norm (x - y)) 2"
  assumes hA_poly: "(\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = A)"
  assumes hA_annulus: "\<exists>A0::(real^2) set. geotop_is_k_annulus 1 A0 \<and>
  ...
  assumes hA_in_BdN: "A \<subseteq> geotop_manifold_boundary N (\<lambda>x y. norm (x - y))"
  assumes hJ: "geotop_is_polygon J" and hJ': "geotop_is_polygon J'"
  assumes hJ_in_A: "J \<subseteq> geotop_manifold_interior A (\<lambda>x y. norm (x - y))"
  assumes hJ'_in_A: "J' \<subseteq> geotop_manifold_interior A (\<lambda>x y. norm (x - y))"
  assumes hJ_essential: "\<not> (\<exists>D. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2
                  \<and> D \<subseteq> geotop_manifold_interior A (\<lambda>x y. norm (x - y))
                  \<and> geotop_frontier UNIV geotop_euclidean_topology D = J)"
  assumes hJ'_essential: "\<not> (\<exists>D. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2
                  \<and> D \<subseteq> geotop_manifold_interior A (\<lambda>x y. norm (x - y))
                  \<and> geotop_frontier UNIV geotop_euclidean_topology D = J')"
  assumes hW_open: "W \<in> geotop_euclidean_topology" and hA_in_W: "A \<subseteq> W"
  shows "\<exists>h. top1_homeomorphism_on M3 (subspace_topology UNIV geotop_euclidean_topology M3)
              M3 (subspace_topology UNIV geotop_euclidean_topology M3) h \<and>
              ...
              (\<forall>P\<in>M3 - W. h P = P)"
proof -
  (** (1) By Theorem 27.2 in A, there is a cellular-PLH chain h_A: A \<leftrightarrow> A with h_A(J) = J'
         and h_A | Bd A = identity. **)
  obtain h\<^sub>A where h_A_cellular:
    "top1_homeomorphism_on A (subspace_topology UNIV geotop_euclidean_topology A)
        A (subspace_topology UNIV geotop_euclidean_topology A) h\<^sub>A \<and>
     h\<^sub>A ` J = J' \<and>
     (\<forall>P\<in>geotop_manifold_boundary A (\<lambda>x y. norm (x - y)). h\<^sub>A P = P)" sorry
  (** (2) Apply Theorem 27.1 to each cellular step of h_A: each cellular PLH of A has a
         PLH extension to M^3, supported inside W, fixing Bd N. Compose the extensions. **)
  have h_extend:
...
theorem Theorem_GT_27_5_Dehn:
  ...
  assumes hM3_mfd: "geotop_n_manifold_with_boundary_on M3 (\<lambda>x y. norm (x - y)) 3"
  assumes hM3_poly: "(\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = M3)"
  assumes hD: "geotop_is_singular_2cell M3 (subspace_topology UNIV geotop_euclidean_topology M3)
             D \<Delta> (subspace_topology UNIV geotop_euclidean_topology \<Delta>)"
  assumes hD_bdry_embed: "top1_homeomorphism_on (geotop_frontier UNIV geotop_euclidean_topology \<Delta>)
             (subspace_topology UNIV geotop_euclidean_topology
                (geotop_frontier UNIV geotop_euclidean_topology \<Delta>))
  ...
  shows "\<exists>\<Delta>1. geotop_is_n_cell \<Delta>1 (subspace_topology UNIV geotop_euclidean_topology \<Delta>1) 2 \<and>
              ...
              geotop_frontier UNIV geotop_euclidean_topology \<Delta>1
                = D ` (geotop_frontier UNIV geotop_euclidean_topology \<Delta>)"
proof -
  (** (1) Let C = D(Bd \<Delta>) be the boundary polygon; it is embedded in M^3 (by hypothesis).
         Take a regular neighbourhood N of the image of D: N = N(D(\<Delta>)) in a subdivision of
         a triangulation K. **)
  obtain N where hN:
    "geotop_n_manifold_with_boundary_on N (\<lambda>x y. norm (x - y)) 3 \<and>
     D ` \<Delta> \<subseteq> N \<and> N \<subseteq> M3" sorry
  (** (2) C is contractible in N (via D), hence (Theorem 24.12) N(C) is a CST. In Bd(N(C))
         there is an annulus A containing C as a core polygon. **)
  have h_CST_annulus:
    "\<exists>NC A. geotop_is_CST NC \<and>
            (\<exists>A0::(real^2) set. geotop_is_k_annulus 1 A0 \<and>
              (\<exists>f. top1_homeomorphism_on A0
                 (subspace_topology UNIV geotop_euclidean_topology A0)
                 A (subspace_topology UNIV geotop_euclidean_topology A) f)) \<and>
            D ` (geotop_frontier UNIV geotop_euclidean_topology \<Delta>) \<subseteq> A" sorry
  (** (3) Apply Stallings' Theorem 25.1 / Loop theorem machinery in N to obtain a nonsingular
         polyhedral 2-cell \<Delta>_1 with Bd \<Delta>_1 = C, replacing the singular D. **)
  have h_Dehn:
...
theorem Theorem_GT_28_2:
  ...
  assumes hM3_poly: "\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = M3"
  assumes hS_in_M3: "S \<subseteq> M3"
  assumes hJx_lat: "geotop_is_latitudinal S Jx"
  assumes hJ: "geotop_is_polygon J"
  assumes hJ_in_T: "J \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y))"
  assumes hW_open: "W \<in> geotop_euclidean_topology"
  assumes hT_in_W: "geotop_manifold_boundary S (\<lambda>x y. norm (x - y)) \<subseteq> W"
  shows "\<exists>h. top1_homeomorphism_on M3 (subspace_topology UNIV geotop_euclidean_topology M3)
              M3 (subspace_topology UNIV geotop_euclidean_topology M3) h \<and>
              h ` S = S \<and>
              ...
              (\<forall>P\<in>M3 - W. h P = P)"
proof -
  (** (1) S is a CST (by 28.1) so has a cylindrical diagram; in this diagram J_x is the
         boundary of one base. J meets J_x finitely, possibly with tangencies. **)
  have h_cyl:
    "\<exists>(\<sigma>2::(real^2) set) (\<phi>::(real^2) \<times> real \<Rightarrow> real^3).
        \<phi> ` (\<sigma>2 \<times> {t. 0 \<le> t \<and> t \<le> 1}) = S \<and>
        \<phi> ` (geotop_frontier UNIV geotop_euclidean_topology \<sigma>2 \<times> {0}) = Jx" sorry
...
theorem Theorem_GT_28_3:
  ...
  assumes hS_in_M3: "S \<subseteq> M3"
  assumes hJx_lat: "geotop_is_latitudinal S Jx"
  assumes hJs_fin: "finite Js"
  assumes hJs: "\<forall>J\<in>Js. geotop_is_polygon J \<and>
             J \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y))"
  assumes hJs_disj: "\<forall>J1\<in>Js. \<forall>J2\<in>Js. J1 \<noteq> J2 \<longrightarrow> J1 \<inter> J2 = {}"
  assumes hW_open: "W \<in> geotop_euclidean_topology"
  assumes hT_in_W: "geotop_manifold_boundary S (\<lambda>x y. norm (x - y)) \<subseteq> W"
  shows "\<exists>h. top1_homeomorphism_on M3 (subspace_topology UNIV geotop_euclidean_topology M3)
              M3 (subspace_topology UNIV geotop_euclidean_topology M3) h \<and>
              ...
              (\<forall>P\<in>M3 - W. h P = P)"
proof -
  (** (1) Apply Theorem 28_2 to each J_i in turn; each PLH h_i supported in a disjoint
         neighbourhood W_i of J_i inside W. Compose sequentially. **)
  have h_per_J:
    "\<forall>J\<in>Js. \<exists>hJ. top1_homeomorphism_on M3 (subspace_topology UNIV geotop_euclidean_topology M3)
                    M3 (subspace_topology UNIV geotop_euclidean_topology M3) hJ \<and>
                 hJ ` S = S \<and>
...
theorem Theorem_GT_28_4:
  ...
  assumes hJx_lat: "geotop_is_latitudinal S Jx"
  assumes hJ_std: "geotop_in_standard_position S J Jx"
...
proof -
  ...
  (** (2) Parametrise J as a closed path p_J based at P_0, and J_x as p_Y. On S (CST)
         ...
         where n is the signed count (matching |J \<cap> J_x| with orientation). **)
  have h_paths:
    "\<forall>P\<^sub>0\<in>J. \<exists>pJ pY. geotop_closed_path_on S
              (subspace_topology UNIV geotop_euclidean_topology S) P\<^sub>0 pJ \<and>
              geotop_closed_path_on S
...
theorem Theorem_GT_28_5:
  ...
  assumes hJ: "geotop_is_polygon J"
  assumes hJ_in_T: "J \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y))"
  assumes hJ_null_on_S: "\<exists>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
             ...
             geotop_path_equiv S (subspace_topology UNIV geotop_euclidean_topology S) P\<^sub>0 p
               (\<lambda>t. P\<^sub>0)"
  assumes hJ_not_null_on_T: "\<not> (\<exists>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
...
proof -
  (** (1) Put J in standard position relative to J_x (Theorem 28_2). Count crossings n
         = |J \<cap> J_x|. **)
  have h_std:
    "\<exists>J'. geotop_in_standard_position S J' Jx \<and>
          (\<exists>h. top1_homeomorphism_on S (subspace_topology UNIV geotop_euclidean_topology S)
                 S (subspace_topology UNIV geotop_euclidean_topology S) h \<and>
               h ` J = J')" sorry
...
theorem Theorem_GT_28_6:
  ...
  assumes hJs_fin: "finite Js" and hJs_card: "2 \<le> card Js"
  assumes hJs: "\<forall>J\<in>Js. geotop_is_polygon J \<and>
             J \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y)) \<and>
...
proof -
  (** (1) T = Bd S is a torus. Each J_i is a non-contractible polygon on T, hence
         (by Theorem 27.3 in T) a latitudinal/longitudinal cycle. The J_i are disjoint
         parallel cycles. **)
  have h_parallel_cycles:
...
theorem Theorem_GT_28_7:
  ...
  assumes hJ: "geotop_is_polygon J"
  assumes hJ_in_T: "J \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y))"
  assumes hJ_essential: "\<exists>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
...
proof -
  (** (1) T = Bd S is a torus. J non-contractible on T is an essential polygon. Take a
         thin regular neighbourhood B of J in T (small tubular neighbourhood along J). **)
  obtain B where hB:
    "(\<exists>K. geotop_is_complex K \<and> geotop_polyhedron K = J) \<and>
     J \<subseteq> geotop_top_interior (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))
         (subspace_topology UNIV geotop_euclidean_topology
            (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))) B" sorry
  (** (2) T - B is the complement of a tubular neighbourhood, which cuts T along the
         essential curve J. The result is an annulus A (by Theorem 28_6 applied to
         the parallel pair {Bd B_1, Bd B_2} of J's regular-nbhd boundary). **)
  have h_annulus_complement:
    "\<exists>A::(real^2) set. geotop_is_k_annulus 1 A \<and>
        (\<exists>f. top1_homeomorphism_on A
                (subspace_topology UNIV geotop_euclidean_topology A)
                (closure_on (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))
...
theorem Theorem_GT_28_8:
  ...
  assumes hJs: "\<forall>J\<in>Js. geotop_is_polygon J \<and>
             ...
             (\<forall>P\<^sub>0\<in>J. \<forall>p. p ` {0..1} = J \<and>
                ...
                \<longrightarrow> \<not> geotop_path_equiv S
                      ...
                      P\<^sub>0 p (\<lambda>t. P\<^sub>0))"
  assumes hJs_disj: "\<forall>J1\<in>Js. \<forall>J2\<in>Js. J1 \<noteq> J2 \<longrightarrow> J1 \<inter> J2 = {}"
  assumes hUnion_gen: "\<exists>P\<^sub>0\<in>\<Union>Js. \<exists>p. p ` {0..1} \<subseteq> \<Union>Js \<and>
              geotop_closed_path_on S
  ...
  shows "\<forall>J\<in>Js. \<exists>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
              ...
              (\<forall>q. geotop_closed_path_on S
                ...
                \<in> geotop_pi S (subspace_topology UNIV geotop_euclidean_topology S) P\<^sub>0)"
proof -
  (** (1) By 28_4/28_5, each non-contractible polygon J_i on T is latitudinal with some
  ...
  have h_each_generator:
    "\<forall>J\<in>Js. \<exists>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
              geotop_closed_path_on S
                (subspace_topology UNIV geotop_euclidean_topology S) P\<^sub>0 p \<and>
              (\<forall>q. geotop_closed_path_on S
                     (subspace_topology UNIV geotop_euclidean_topology S) P\<^sub>0 q \<longrightarrow>
...
theorem Theorem_GT_28_9:
  ...
  assumes hJ: "geotop_is_polygon J"
  assumes hJ_in_T: "J \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y))"
  assumes hJ_null: "\<exists>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
             geotop_path_equiv (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))
                (subspace_topology UNIV geotop_euclidean_topology
                   (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))) P\<^sub>0 p (\<lambda>t. P\<^sub>0)"
...
proof -
  (** (1) T = Bd S is a torus. J null on T means J is contractible to a point in T. **)
  have h_null_curve:
    "\<exists>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
         geotop_path_equiv (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))
           (subspace_topology UNIV geotop_euclidean_topology
              (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))) P\<^sub>0 p (\<lambda>t. P\<^sub>0)" sorry
  (** (2) On the torus T, any null-homotopic polygon bounds a 2-cell in T (classical
         result: \<pi>_1(T) = Z \<times> Z and null curves bound disks). Extract D \<subseteq> T. **)
  have h_bounds_cell:
    "\<exists>D. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2 \<and>
         D \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y)) \<and>
...
theorem Theorem_GT_28_10:
  ...
  assumes hS: "geotop_is_CST S"
  assumes hK_in_T: "K \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y))"
  assumes hK_poly: "\<exists>L. geotop_is_complex L \<and> geotop_polyhedron L = K"
  assumes hK_gen: "\<exists>P\<^sub>0\<in>K. \<exists>p. p ` {0..1} \<subseteq> K \<and>
             geotop_closed_path_on S
               (subspace_topology UNIV geotop_euclidean_topology S) P\<^sub>0 p"
  assumes hJ: "geotop_is_polygon J"
  assumes hJ_ext: "J \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y)) - K"
  assumes hJ_no_cell: "\<nexists>D. geotop_is_n_cell D (subspace_topology UNIV geotop_euclidean_topology D) 2 \<and>
              D \<subseteq> geotop_manifold_boundary S (\<lambda>x y. norm (x - y)) \<and>
              geotop_frontier UNIV geotop_euclidean_topology D = J"
  shows "\<forall>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
              geotop_closed_path_on S
                (subspace_topology UNIV geotop_euclidean_topology S) P\<^sub>0 p"
proof -
  (** (1) J doesn't bound a 2-cell on T \<Rightarrow> J is non-contractible on T (Theorem 28_9
         contrapositive). **)
  have h_J_essential:
    "\<forall>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
         ...
         \<not> geotop_path_equiv (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))
             (subspace_topology UNIV geotop_euclidean_topology
                (geotop_manifold_boundary S (\<lambda>x y. norm (x - y)))) P\<^sub>0 p (\<lambda>t. P\<^sub>0)" sorry
  (** (2) Since K already carries a generator and J is disjoint from K, J must be
         parallel to K. By Theorem 28_8 applied to {K-equivalent, J}, J itself carries
         a generator class. **)
  have h_J_generator:
    "\<forall>P\<^sub>0\<in>J. \<exists>p. p ` {0..1} = J \<and>
         geotop_closed_path_on S
...
theorem Theorem_GT_28_14:
  fixes J :: "(real^3) set"
  assumes hJ: "geotop_is_polygon J"
  (** Moise: H\<^sub>1(R\<^sup>3 - J) \<cong> Z. Without first-homology infrastructure, we express
      this as an isomorphism between pi(R\<^sup>3 - J) (abelianized modulo commutators)
      and the free abelian group on one generator. We use the
      \<open>bij_betw \<Phi> (free_group {a}) (pi ...)\<close> pattern from Theorem_GT_15_4. Note
      this strict form (bij_betw with free group) is in fact \<pi> \<cong> Z, which is
      strictly false for knotted polygons (their \<pi> is non-abelian). The correct
      Moise content is about H\<^sub>1 (abelianization); its formalization awaits H\<^sub>1
      infrastructure. Until then, the theorem is stated with the Moise content
      and remains sorry. **)
  shows "\<exists>P\<^sub>0\<in>UNIV - J. \<exists>(G::nat set) (\<Phi>::(nat \<times> int) list set \<Rightarrow> (real \<Rightarrow> real^3) set).
           card G = 1 \<and>
...
proof -
  ...
  have h_PL_torus:
    "\<exists>T'. geotop_is_torus T' \<and>
          ...
          T' \<subseteq> geotop_manifold_interior Y (\<lambda>x y. norm (x - y)) \<and>
          geotop_separates_in UNIV geotop_euclidean_topology T' T0 T1" sorry
  show ?thesis using h_PL_torus by (by100 blast)
qed
