...
focus_target() {
  case "$1" in
    ...
    graph-local-side-witness)
      printf '%s\t%s\t%s\n' \
        dev34_prefix_graph/cache/GeoTop_3_4_Prefix_Graph_Cache.thy \
        geotop_branch_vertex_local_side_witness_from_split_side_prefix \
        'branch vertex local-side witness from split-side data'
      ;;
    graph-germs-bridge)
      printf '%s\t%s\t%s\n' \
        dev34_prefix_graph/cache/GeoTop_3_4_Prefix_Graph_Cache.thy \
        geotop_branch_vertex_three_germs_same_side_component_prefix \
        'branch vertex three germs same side component local first entry'
      ;;
    graph-branch-local)
      printf '%s\t%s\t%s\n' \
        dev34_prefix_graph/cache/GeoTop_3_4_Prefix_Graph_Cache.thy \
        geotop_branch_vertex_local_disconnects_finite_linear_graph_prefix \
        'branch vertex local disconnects finite linear graph'
      ;;
    ...
    mid-fold-one)
      printf '%s\t%s\t%s\n' \
        ...
        'Figure 3.3 one boundary edge supported fold'
      ;;
    mid-fold-two)
      printf '%s\t%s\t%s\n' \
        dev34_prefix_mid/GeoTop_3_4_Prefix_Mid.thy \
        geotop_free_triangle_two_boundary_edges_supported_inverse_fold_prefix \
        'Figure 3.3 two boundary edge inverse fold'
      ;;
    mid-fold)
      printf '%s\t%s\t%s\n' \
        dev34_prefix_mid/GeoTop_3_4_Prefix_Mid.thy \
    ...
    prefix-d44-raw)
      printf '%s\t%s\t%s\n' \
        dev34_prefix/GeoTop_3_4_Prefix.thy \
        geotop_polygon_two_endpoint_arcs_selected_carrier_frontier_local_graph_corridor_raw_moise_step_prefix \
        'D44 raw Moise local graph corridor regular neighborhood paragraph'
      ;;
    prefix-d44-root)
      printf '%s\t%s\t%s\n' \
        dev34_prefix/GeoTop_3_4_Prefix.thy \
        geotop_polygon_two_endpoint_arcs_selected_carrier_frontier_sphere_same_component_root_book_step_prefix \
        'D44 root selected carrier frontier sphere same component postprocessing'
      ;;
    prefix-d44-book)
      printf '%s\t%s\t%s\n' \
        dev34_prefix/GeoTop_3_4_Prefix.thy \
