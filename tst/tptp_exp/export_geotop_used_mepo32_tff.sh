...
# theorem dependencies recorded in Isabelle proof terms, padded with a small
# number of MePo-selected facts that are not already proof dependencies.
#
# Usage:
#   cd /project/tst
#   tptp_exp/export_geotop_used_mepo32_tff.sh [OUT_DIR]
#
# Environment:
#   GEOTOP_USED_MEPO_TPTP_EXTRA_FACTS  MePo padding budget, default 32
...
OUT_DIR="${1:-$PROJECT_DIR/tptp_probs_geotop_used_mepo32}"
EXTRA_FACTS="${GEOTOP_USED_MEPO_TPTP_EXTRA_FACTS:-32}"
LIMIT="${GEOTOP_USED_MEPO_TPTP_LIMIT:-0}"
SCOPE="${GEOTOP_USED_MEPO_TPTP_SCOPE:-all_geotop}"

mkdir -p "$OUT_DIR"
OUT_DIR="$(cd "$OUT_DIR" && pwd)"

TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/geotop_used_mepo_tff_export.XXXXXX")"
...
if scope not in theories:
    raise SystemExit(f"Unknown GEOTOP_USED_MEPO_TPTP_SCOPE={scope!r}; expected geotop or all_geotop")
...
cat > "$TMP_DIR/GeoTop_Used_MePo_TFF_Export.thy" <<EOF
theory GeoTop_Used_MePo_TFF_Export
...
$SOURCE_DECLS
  ...
  val _ =
    File.write manifest
      "file\\ttheorem\\tproof_deps\\tresolved_deps\\tmissing_deps\\tmepo_candidates\\tmepo_extra\\ttotal_facts\\toracles\\thas_skip_proof\\n"
  ...
  val params =
    Sledgehammer_Commands.default_params thy
      [("provers", "e"), ("type_enc", "mono_native"), ("lam_trans", "lifting"),
       ("fact_filter", "mepo"), ("max_facts", string_of_int extra_facts), ("timeout", "1")]
  ...
  fun export_one (name, th) =
    let
      ...
      val mepo_candidates =
        Sledgehammer_MePo.mepo_suggested_facts ctxt params extra_facts NONE hyp_ts concl_t usable_facts
      fun already_have_prop fact_th =
        exists (fn (_, dep_th) => Thm.eq_thm_prop (fact_th, dep_th)) resolved_deps
      val mepo_extra =
        mepo_candidates
        |> filter_out (fn (_, fact_th) => already_have_prop fact_th)
        |> take extra_facts
      val atp_facts =
        (resolved_deps |> map (fn (dep_name, dep_th) =>
          ((Thm_Name.short dep_name, (ATP_Problem_Generate.Global, ATP_Problem_Generate.General)),
            Thm.prop_of dep_th))) @
        (mepo_extra |> map (fn ((fact_name, stature), fact_th) =>
      ...
      val _ =
        File.append manifest
          (file_name ^ "\\t" ^ manifest_field target_name ^ "\\t" ^
            ...
            string_of_int (length mepo_candidates) ^ "\\t" ^
            string_of_int (length mepo_extra) ^ "\\t" ^
            string_of_int (length atp_facts) ^ "\\t" ^
            string_of_int (length oracles) ^ "\\t" ^
            Bool.toString has_skip ^ "\\n")
    in
      writeln ("exported " ^ target_name ^ " with " ^
        string_of_int (length resolved_deps) ^ " proof deps + " ^
        string_of_int (length mepo_extra) ^ " MePo facts to " ^ Path.print file)
    end
    handle
  ...
  val _ =
    writeln ("Used-dependency + MePo TFF export completed: " ^ string_of_int (length targets) ^
      " targets from " ^ string_of_int (length source_decls) ^
      " active source declarations in scope $SCOPE with MePo padding " ^
      string_of_int extra_facts)
in
  ()
end
\<close>
...
/project/bin/isabelle process_theories -d . -l GeoTop -o quick_and_dirty -O \
  -f "$TMP_DIR/GeoTop_Used_MePo_TFF_Export.thy"
