...
lemma geotop_2simplex_delete_edge_face_connected_prefix:
  fixes \<sigma> e :: "(real^2) set"
  assumes h\<sigma>2: "geotop_simplex_dim \<sigma> 2"
  assumes hedge: "geotop_is_edge e"
  assumes hface: "geotop_is_face e \<sigma>"
  shows "top1_connected_on (\<sigma> - e)
    (subspace_topology UNIV geotop_euclidean_topology (\<sigma> - e))"
proof -
  have hfront:
    "frontier \<sigma> = \<Union>{d. geotop_is_edge d \<and> geotop_is_face d \<sigma>}"
    by (rule geotop_2simplex_frontier_eq_edge_faces_prefix[OF h\<sigma>2])
  have he_front: "e \<subseteq> frontier \<sigma>"
  proof
    fix x
    assume hx: "x \<in> e"
    have "x \<in> \<Union>{d. geotop_is_edge d \<and> geotop_is_face d \<sigma>}"
      using hx hedge hface by (by100 blast)
    thus "x \<in> frontier \<sigma>"
      using hfront by (by100 simp)
  qed
  have hconv: "convex \<sigma>"
    by (rule geotop_simplex_dim_convex_HOL_prefix[OF h\<sigma>2])
  have hconv_int: "convex (interior \<sigma>)"
    by (rule convex_interior[OF hconv])
  have hconn_int: "connected (interior \<sigma>)"
    by (rule convex_connected[OF hconv_int])
  have hint_sub_delete: "interior \<sigma> \<subseteq> \<sigma> - e"
  proof
    fix x
    assume hx_int: "x \<in> interior \<sigma>"
    have hx\<sigma>: "x \<in> \<sigma>"
      using hx_int interior_subset by (by100 blast)
    have hx_not_e: "x \<notin> e"
    proof
      assume hx_e: "x \<in> e"
      have hx_front: "x \<in> frontier \<sigma>"
        using he_front hx_e by (by100 blast)
      have "x \<notin> interior \<sigma>"
        using hx_front unfolding Elementary_Topology.frontier_def by (by100 simp)
      thus False
        using hx_int by (by100 blast)
    qed
    show "x \<in> \<sigma> - e"
      using hx\<sigma> hx_not_e by (by100 blast)
  qed
  have hclosure_int: "closure (interior \<sigma>) = \<sigma>"
    by (rule geotop_2simplex_closure_HOL_interior_prefix[OF h\<sigma>2])
  have hdelete_sub_closure: "\<sigma> - e \<subseteq> closure (interior \<sigma>)"
    using hclosure_int by (by100 blast)
  have hconn_HOL: "connected (\<sigma> - e)"
    by (rule connected_intermediate_closure
        [OF hconn_int hint_sub_delete hdelete_sub_closure])
  show ?thesis
    using hconn_HOL top1_connected_on_geotop_iff_connected by (by100 blast)
qed
...
lemma geotop_free_2_simplex_canonical_selected_edge_contact_prefix:
  fixes K :: "(real^2) set set" and J \<sigma>\<^sub>2 :: "(real^2) set"
  assumes hJ: "geotop_is_polygon J"
  assumes hK: "geotop_is_complex K"
  assumes hK_poly: "geotop_polyhedron K =
      closure_on UNIV geotop_euclidean_topology (geotop_polygon_interior J)"
  ...
  shows "finite {e\<in>K. geotop_is_edge e \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J}
    \<and> card {e\<in>K. geotop_is_edge e \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J} \<le> 2
    \<and> \<sigma>\<^sub>2 \<inter> J =
      \<Union>{e\<in>K. geotop_is_edge e \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J}"
  (**
    Canonical form of a free side witness in a multi-triangle disk.  The
    definition of freeness may choose an arbitrary allowed selected-edge
    family; this lemma replaces it by the canonical family of all selected
    boundary edge faces, using the already-proved at-most-two bound. **)
proof -
  let ?C = "{e\<in>K. geotop_is_edge e \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J}"
  have hC_data: "finite ?C \<and> card ?C \<le> 2"
    by (rule geotop_polygon_disk_multi_2simplex_selected_boundary_edges_card_le2_prefix
        [OF hJ hK hK_poly h\<sigma>K h\<sigma>2 hT_gt1])
  have hC_fin: "finite ?C"
    using hC_data by (by100 blast)
  have hC_card: "card ?C \<le> 2"
    using hC_data by (by100 blast)
  obtain E where hEsub: "E \<subseteq> K"
    and hEallowed:
      "E = {}
        \<or> (\<exists>e. E = {e} \<and> geotop_is_edge e
          \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J)
        \<or> (\<exists>e1 e2. E = {e1, e2} \<and> e1 \<noteq> e2
    ...
    using hfree unfolding geotop_free_2_simplex_def by (elim exE conjE)
  have hE_sub_C: "E \<subseteq> ?C"
    using hEsub hEallowed
    apply (elim disjE exE conjE)
    apply (by100 blast)
    apply (by100 blast)
    apply (by100 blast)
    done
  have hcontact_can: "\<sigma>\<^sub>2 \<inter> J = \<Union>?C"
  proof
    show "\<sigma>\<^sub>2 \<inter> J \<subseteq> \<Union>?C"
      using hcontact hE_sub_C by (by100 blast)
    show "\<Union>?C \<subseteq> \<sigma>\<^sub>2 \<inter> J"
      by (rule geotop_selected_boundary_edge_set_union_subset_contact_prefix)
  qed
  show ?thesis
    using hC_fin hC_card hcontact_can by (by100 blast)
qed
...
lemma geotop_nonfree_boundary_triangle_contact_on_nonbase_segment_off_base_prefix:
  fixes J \<theta> :: "(real^2) set" and K :: "(real^2) set set"
  ...
  shows "\<exists>x. x \<in> \<theta> \<inter> J
      \<and> x \<notin> closed_segment v\<^sub>0 v\<^sub>1
      \<and> x \<in> (closed_segment v\<^sub>0 v\<^sub>2 - {v\<^sub>0})
          \<union> (closed_segment v\<^sub>1 v\<^sub>2 - {v\<^sub>1})"
  (**
    Nonfree boundary-triangle contact package for Moise Figure 3.2: if the
    base edge \<open>v\<^sub>0v\<^sub>1\<close> is on the polygon boundary but the triangle is not free,
    the extra boundary contact occurs on one of the two nonbase sides and not
    on the base segment. **)
proof -
  ...
  have hE_allowed:
    "?E = {} \<or>
     (\<exists>e. ?E = {e} \<and> geotop_is_edge e \<and> geotop_is_face e \<theta> \<and> e \<subseteq> J) \<or>
     (\<exists>e1 e2. ?E = {e1, e2} \<and> e1 \<noteq> e2 \<and>
        geotop_is_edge e1 \<and> geotop_is_edge e2 \<and>
  ...
  have hE_subset_K: "?E \<subseteq> K"
    by (by100 simp)
  have hE_union_sub_\<theta>J: "\<Union>?E \<subseteq> \<theta> \<inter> J"
    by (rule geotop_selected_boundary_edge_set_union_subset_contact_prefix)
  have h\<theta>_contact_outside_selected:
    "\<exists>x. x \<in> \<theta> \<inter> J \<and> x \<notin> \<Union>?E"
    by (rule geotop_nonfree_selected_edges_contact_outside_prefix
        [OF h\<theta>K h\<theta>2 hE_subset_K hE_allowed h\<theta>_not_free
          hE_union_sub_\<theta>J])
...
lemma geotop_chord_side_complexes_exact_and_strict_from_carriers_prefix:
  fixes J J\<^sub>1 J\<^sub>2 :: "(real^2) set"
    and K L\<^sub>1 L\<^sub>2 :: "(real^2) set set"
    and \<rho>\<^sub>1 \<rho>\<^sub>2 :: "(real^2) set"
  assumes hK: "geotop_is_complex K"
  assumes hK_fin: "finite K"
  assumes hK_poly:
    "geotop_polyhedron K =
      closure_on UNIV geotop_euclidean_topology (geotop_polygon_interior J)"
  assumes hclosure_split:
  ...
  assumes hL\<^sub>2_def:
    "L\<^sub>2 =
      {\<rho>\<in>K. \<rho> \<subseteq>
        closure_on UNIV geotop_euclidean_topology
          (geotop_polygon_interior J\<^sub>2)}"
  assumes hcarrier_side1:
    "\<forall>x\<in>closure_on UNIV geotop_euclidean_topology
        (geotop_polygon_interior J\<^sub>1).
        geotop_K_carrier K x \<subseteq>
          closure_on UNIV geotop_euclidean_topology
            (geotop_polygon_interior J\<^sub>1)"
  assumes hcarrier_side2:
    "\<forall>x\<in>closure_on UNIV geotop_euclidean_topology
        (geotop_polygon_interior J\<^sub>2).
        geotop_K_carrier K x \<subseteq>
          closure_on UNIV geotop_euclidean_topology
            (geotop_polygon_interior J\<^sub>2)"
  assumes h\<rho>\<^sub>1_T:
    "\<rho>\<^sub>1 \<in> {\<rho>\<in>K. geotop_simplex_dim \<rho> 2}"
  assumes h\<rho>\<^sub>1_not_T\<^sub>1:
    "\<rho>\<^sub>1 \<notin> {\<rho>\<in>L\<^sub>1. geotop_simplex_dim \<rho> 2}"
  assumes h\<rho>\<^sub>2_T:
    "\<rho>\<^sub>2 \<in> {\<rho>\<in>K. geotop_simplex_dim \<rho> 2}"
  assumes h\<rho>\<^sub>2_not_T\<^sub>2:
    "\<rho>\<^sub>2 \<notin> {\<rho>\<in>L\<^sub>2. geotop_simplex_dim \<rho> 2}"
  assumes hT\<^sub>1_gt1:
    "card {\<rho>\<in>L\<^sub>1. geotop_simplex_dim \<rho> 2} > 1"
  assumes hT\<^sub>2_gt1:
    "card {\<rho>\<in>L\<^sub>2. geotop_simplex_dim \<rho> 2} > 1"
  shows
    ...
    \<and> geotop_polyhedron L\<^sub>2 =
       closure_on UNIV geotop_euclidean_topology
         (geotop_polygon_interior J\<^sub>2)
    \<and> card {\<rho>\<in>L\<^sub>1. geotop_simplex_dim \<rho> 2}
        < card {\<rho>\<in>K. geotop_simplex_dim \<rho> 2}
    \<and> card {\<rho>\<in>L\<^sub>2. geotop_simplex_dim \<rho> 2}
        < card {\<rho>\<in>K. geotop_simplex_dim \<rho> 2}
    \<and> card {\<rho>\<in>L\<^sub>1. geotop_simplex_dim \<rho> 2} > 1
    \<and> card {\<rho>\<in>L\<^sub>2. geotop_simplex_dim \<rho> 2} > 1"
  (**
    Moise Figure 3.2 bookkeeping layer: once each closed side disk has its
    original K-carriers inside the same side and each side omits an explicit
    parent 2-simplex, the restricted side complexes triangulate the two
    closed subdisks and have strictly fewer 2-simplexes than the parent. **)
proof -
  let ?B\<^sub>1 =
  ...
  have hL\<^sub>2_poly_eq: "geotop_polyhedron L\<^sub>2 = ?B\<^sub>2"
  ...
  have hT\<^sub>2_lt_T: "card ?T\<^sub>2 < card ?T"
    by (rule geotop_finite_subset_card_lt_if_omits_member_prefix
        [OF hT_fin hT\<^sub>2_sub_T h\<rho>\<^sub>2_T h\<rho>\<^sub>2_not_T\<^sub>2])
  show ?thesis
...
proof -
  obtain V\<^sub>\<tau> W\<^sub>\<rho> where h\<tau>V: "geotop_simplex_vertices \<tau> V\<^sub>\<tau>"
    ...
    by (rule geotop_face_witness_simplex_vertices_prefix[OF h\<rho>\<tau>])
  obtain V\<^sub>\<sigma> W\<^sub>\<tau> where h\<sigma>V: "geotop_simplex_vertices \<sigma> V\<^sub>\<sigma>"
    ...
    by (rule geotop_face_witness_simplex_vertices_prefix[OF h\<tau>\<sigma>])
...
lemma geotop_complex_union_subcomplexes_in_ambient_prefix:
  ...
  assumes hM_faces: "\<And>\<sigma> \<tau>. \<sigma> \<in> M \<Longrightarrow> geotop_is_face \<tau> \<sigma> \<Longrightarrow> \<tau> \<in> M"
  assumes hLsub: "L \<subseteq> K"
  assumes hMsub: "M \<subseteq> K"
  shows "geotop_is_complex (L \<union> M)"
proof -
  have hLMsub: "L \<union> M \<subseteq> K"
    using hLsub hMsub by (by100 blast)
  have hsimplex: "\<forall>\<sigma>\<in>L \<union> M. geotop_is_simplex \<sigma>"
  proof
    fix \<sigma>
    assume h\<sigma>LM: "\<sigma> \<in> L \<union> M"
    have h\<sigma>K: "\<sigma> \<in> K"
      using hLMsub h\<sigma>LM by (by100 blast)
    show "geotop_is_simplex \<sigma>"
      using geotop_is_complex_simplex[OF hK] h\<sigma>K by (by100 blast)
  qed
  have hfaces:
    "\<forall>\<sigma>\<in>L \<union> M. \<forall>\<tau>. geotop_is_face \<tau> \<sigma> \<longrightarrow> \<tau> \<in> L \<union> M"
  proof (intro ballI allI impI)
    fix \<sigma> \<tau>
    assume h\<sigma>: "\<sigma> \<in> L \<union> M"
    assume hface: "geotop_is_face \<tau> \<sigma>"
    show "\<tau> \<in> L \<union> M"
    proof (rule UnE[OF h\<sigma>])
      assume h\<sigma>L: "\<sigma> \<in> L"
      have "\<tau> \<in> L"
        using hL h\<sigma>L hface unfolding geotop_is_complex_def by (by100 blast)
      thus ?thesis by (by100 blast)
    next
      assume h\<sigma>M: "\<sigma> \<in> M"
      have "\<tau> \<in> M"
        by (rule hM_faces[OF h\<sigma>M hface])
      thus ?thesis by (by100 blast)
    qed
  qed
  have hinter:
    "\<forall>\<sigma>\<in>L \<union> M. \<forall>\<tau>\<in>L \<union> M.
      \<sigma> \<inter> \<tau> \<noteq> {} \<longrightarrow>
        geotop_is_face (\<sigma> \<inter> \<tau>) \<sigma> \<and> geotop_is_face (\<sigma> \<inter> \<tau>) \<tau>"
  proof (intro ballI impI)
    fix \<sigma> \<tau>
    assume h\<sigma>LM: "\<sigma> \<in> L \<union> M"
    assume h\<tau>LM: "\<tau> \<in> L \<union> M"
    assume hinter_ne: "\<sigma> \<inter> \<tau> \<noteq> {}"
    have h\<sigma>K: "\<sigma> \<in> K"
      using hLMsub h\<sigma>LM by (by100 blast)
    have h\<tau>K: "\<tau> \<in> K"
      using hLMsub h\<tau>LM by (by100 blast)
    show "geotop_is_face (\<sigma> \<inter> \<tau>) \<sigma> \<and> geotop_is_face (\<sigma> \<inter> \<tau>) \<tau>"
      using geotop_is_complex_intersection[OF hK] h\<sigma>K h\<tau>K hinter_ne
      by (by100 blast)
  qed
  have hlocal:
    "\<forall>\<sigma>\<in>L \<union> M. \<exists>U. open U \<and> \<sigma> \<subseteq> U \<and>
      finite {\<tau>\<in>L \<union> M. \<tau> \<inter> U \<noteq> {}}"
  proof
    fix \<sigma>
    assume h\<sigma>LM: "\<sigma> \<in> L \<union> M"
    have h\<sigma>K: "\<sigma> \<in> K"
      using hLMsub h\<sigma>LM by (by100 blast)
    have hK_local_all:
      "\<forall>\<sigma>\<in>K. \<exists>U. open U \<and> \<sigma> \<subseteq> U \<and>
        finite {\<tau>\<in>K. \<tau> \<inter> U \<noteq> {}}"
      by (rule geotop_is_complex_locally_finite[OF hK])
    have hK_local_\<sigma>:
      "\<exists>U. open U \<and> \<sigma> \<subseteq> U \<and> finite {\<tau>\<in>K. \<tau> \<inter> U \<noteq> {}}"
      by (rule bspec[OF hK_local_all h\<sigma>K])
    obtain U where hUopen: "open U" and h\<sigma>U: "\<sigma> \<subseteq> U"
      and hUfin: "finite {\<tau>\<in>K. \<tau> \<inter> U \<noteq> {}}"
      using hK_local_\<sigma> by (by100 blast)
    have hsub:
      "{\<tau>\<in>L \<union> M. \<tau> \<inter> U \<noteq> {}} \<subseteq> {\<tau>\<in>K. \<tau> \<inter> U \<noteq> {}}"
      using hLMsub by (by100 blast)
    have hfin: "finite {\<tau>\<in>L \<union> M. \<tau> \<inter> U \<noteq> {}}"
      by (rule finite_subset[OF hsub hUfin])
    show "\<exists>U. open U \<and> \<sigma> \<subseteq> U \<and>
      finite {\<tau>\<in>L \<union> M. \<tau> \<inter> U \<noteq> {}}"
      using hUopen h\<sigma>U hfin by (by100 blast)
  qed
  show ?thesis
    unfolding geotop_is_complex_def using hsimplex hfaces hinter hlocal by (by100 blast)
qed

lemma geotop_theta_face_adjoin_side_complex_prefix:
  fixes K L :: "(real^2) set set" and \<theta> :: "(real^2) set"
  assumes hK: "geotop_is_complex K"
  assumes hL: "geotop_is_complex L"
  assumes hLsub: "L \<subseteq> K"
  assumes h\<theta>K: "\<theta> \<in> K"
  ...
  shows "geotop_is_complex
    ...
    \<and> L \<union> {\<rho>. geotop_is_face \<rho> \<theta> \<or> \<rho> = \<theta>} \<subseteq> K"
proof -
  ...
  proof
    ...
    show "\<rho> \<in> K"
    proof (rule disjE)
      show "geotop_is_face \<rho> \<theta> \<or> \<rho> = \<theta>"
  ...
  qed
  have hF_faces:
    "\<And>\<sigma> \<rho>. \<sigma> \<in> ?F \<Longrightarrow> geotop_is_face \<rho> \<sigma> \<Longrightarrow> \<rho> \<in> ?F"
  proof -
    fix \<sigma> \<rho>
    assume h\<sigma>F: "\<sigma> \<in> ?F"
    assume h\<rho>\<sigma>: "geotop_is_face \<rho> \<sigma>"
    have hcase: "geotop_is_face \<sigma> \<theta> \<or> \<sigma> = \<theta>"
      using h\<sigma>F by (by100 blast)
    show "\<rho> \<in> ?F"
    proof (rule disjE[OF hcase])
      assume h\<sigma>\<theta>: "geotop_is_face \<sigma> \<theta>"
      have "geotop_is_face \<rho> \<theta>"
        by (rule geotop_is_face_trans_prefix[OF h\<rho>\<sigma> h\<sigma>\<theta>])
      thus ?thesis by (by100 blast)
    next
      assume "\<sigma> = \<theta>"
      thus ?thesis using h\<rho>\<sigma> by (by100 blast)
    qed
  qed
  have hcomplex: "geotop_is_complex (L \<union> ?F)"
    by (rule geotop_complex_union_subcomplexes_in_ambient_prefix
        [OF hK hL hF_faces hLsub hFsub])
  have hsub: "L \<union> ?F \<subseteq> K"
...
proof -
  let ?F = "{\<eta>. geotop_is_face \<eta> \<theta> \<or> \<eta> = \<theta>}"
  let ?T = "{\<eta>\<in>L \<union> ?F. geotop_simplex_dim \<eta> 2}"
  have hF_fin: "finite ?F"
  proof -
    have h\<theta>simplex: "geotop_is_simplex \<theta>"
      by (rule geotop_simplex_dim_imp_is_simplex[OF h\<theta>2])
    have hfaces_fin: "finite {\<eta>. geotop_is_face \<eta> \<theta>}"
      by (rule geotop_simplex_faces_finite[OF h\<theta>simplex])
    have "?F = {\<eta>. geotop_is_face \<eta> \<theta>} \<union> {\<theta>}"
      by (by100 blast)
    thus ?thesis
  ...
  have h\<rho>T: "\<rho> \<in> ?T"
    using h\<rho>L h\<rho>2 by (by100 blast)
  have hpair_sub: "{\<theta>, \<rho>} \<subseteq> ?T"
    using h\<theta>T h\<rho>T by (by100 blast)
  have hpair_card: "card {\<theta>, \<rho>} = 2"
    using h\<rho>ne by (by100 simp)
  have "card {\<theta>, \<rho>} \<le> card ?T"
    by (rule card_mono[OF hT_fin hpair_sub])
  thus ?thesis
...
proof -
  ...
  qed
  have hside1_off_chord_point_in_R\<^sub>1:
    "\<And>x. x \<in> ?B\<^sub>1 \<Longrightarrow> x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow> x \<in> ?R\<^sub>1"
    using hB\<^sub>1_minus_chord_sub_R\<^sub>1 by (by100 blast)
  have hside2_off_chord_point_in_R\<^sub>2:
    "\<And>x. x \<in> ?B\<^sub>2 \<Longrightarrow> x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow> x \<in> ?R\<^sub>2"
    using hB\<^sub>2_minus_chord_sub_R\<^sub>2 by (by100 blast)
  have hcarrier_side1_point_region_cases:
    "\<And>x y. x \<in> ?B\<^sub>1 \<Longrightarrow> y \<in> geotop_K_carrier K x \<Longrightarrow>
      y \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow> y \<in> ?R\<^sub>1 \<or> y \<in> ?R\<^sub>2"
  proof -
    fix x y
    assume hxB: "x \<in> ?B\<^sub>1"
    assume hycarrier: "y \<in> geotop_K_carrier K x"
    assume hy_not_chord: "y \<notin> closed_segment v\<^sub>0 v\<^sub>2"
    have hxK: "x \<in> geotop_polyhedron K"
      using hB\<^sub>1_sub_K hxB by (by100 blast)
    have hcarrier_poly:
      "geotop_K_carrier K x \<subseteq> geotop_polyhedron K"
      by (rule geotop_K_carrier_in_polyhedron[OF hK hK_fin hxK])
    have hy_parent_closure:
      "y \<in> closure_on UNIV geotop_euclidean_topology (geotop_polygon_interior J)"
      using hcarrier_poly hK_poly hycarrier by (by100 blast)
    have hy_parent_minus:
      "y \<in> closure_on UNIV geotop_euclidean_topology
          (geotop_polygon_interior J) - closed_segment v\<^sub>0 v\<^sub>2"
      using hy_parent_closure hy_not_chord by (by100 blast)
    show "y \<in> ?R\<^sub>1 \<or> y \<in> ?R\<^sub>2"
      using hclosure_minus hy_parent_minus by (by100 blast)
  qed
  have hcarrier_side2_point_region_cases:
    "\<And>x y. x \<in> ?B\<^sub>2 \<Longrightarrow> y \<in> geotop_K_carrier K x \<Longrightarrow>
      y \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow> y \<in> ?R\<^sub>1 \<or> y \<in> ?R\<^sub>2"
  proof -
    fix x y
    assume hxB: "x \<in> ?B\<^sub>2"
    ...
    show "y \<in> ?R\<^sub>1 \<or> y \<in> ?R\<^sub>2"
      using hclosure_minus hy_parent_minus by (by100 blast)
  qed
  have hR\<^sub>1_sub_B\<^sub>1: "?R\<^sub>1 \<subseteq> ?B\<^sub>1"
  proof
    fix x
    assume hx: "x \<in> ?R\<^sub>1"
    have hx_cases:
      "x \<in> geotop_polygon_interior J\<^sub>1
        \<or> x \<in> geotop_arc_interior C\<^sub>1 {v\<^sub>0, v\<^sub>2}"
      using hx by (by100 blast)
    show "x \<in> ?B\<^sub>1"
    proof (rule disjE[OF hx_cases])
      assume hxI: "x \<in> geotop_polygon_interior J\<^sub>1"
      have "geotop_polygon_interior J\<^sub>1 \<subseteq> ?B\<^sub>1"
        by (rule subset_closure_on)
      thus ?thesis
        using hxI by (by100 blast)
    next
      assume hxarc: "x \<in> geotop_arc_interior C\<^sub>1 {v\<^sub>0, v\<^sub>2}"
      have "x \<in> C\<^sub>1"
        using hxarc unfolding geotop_arc_interior_def by (by100 blast)
      thus ?thesis
        using hC\<^sub>1_sub_J\<^sub>1 hJ\<^sub>1_sub_B\<^sub>1 by (by100 blast)
    qed
  qed
  have hR\<^sub>2_sub_B\<^sub>2: "?R\<^sub>2 \<subseteq> ?B\<^sub>2"
  proof
    fix x
    assume hx: "x \<in> ?R\<^sub>2"
    have hx_cases:
      "x \<in> geotop_polygon_interior J\<^sub>2
        \<or> x \<in> geotop_arc_interior C\<^sub>2 {v\<^sub>0, v\<^sub>2}"
      using hx by (by100 blast)
    show "x \<in> ?B\<^sub>2"
    proof (rule disjE[OF hx_cases])
      assume hxI: "x \<in> geotop_polygon_interior J\<^sub>2"
      have "geotop_polygon_interior J\<^sub>2 \<subseteq> ?B\<^sub>2"
        by (rule subset_closure_on)
      thus ?thesis
        using hxI by (by100 blast)
    next
      assume hxarc: "x \<in> geotop_arc_interior C\<^sub>2 {v\<^sub>0, v\<^sub>2}"
      have "x \<in> C\<^sub>2"
        using hxarc unfolding geotop_arc_interior_def by (by100 blast)
      thus ?thesis
        using hC\<^sub>2_sub_J\<^sub>2 hJ\<^sub>2_sub_B\<^sub>2 by (by100 blast)
    qed
  qed
  have hcarrier_side1_avoids_chord:
    "\<And>x. x \<in> ?B\<^sub>1 \<Longrightarrow> x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 = {} \<Longrightarrow>
      geotop_K_carrier K x \<subseteq> ?B\<^sub>1"
  proof -
    fix x
    assume hxB: "x \<in> ?B\<^sub>1"
    assume hx_not_chord: "x \<notin> closed_segment v\<^sub>0 v\<^sub>2"
    assume hcarrier_no_chord:
    ...
    have hcarrier_path_conn:
      "top1_path_connected_on (geotop_K_carrier K x)
        (subspace_topology UNIV geotop_euclidean_topology
          (geotop_K_carrier K x))"
      by (rule Theorem_GT_1_3[OF hcarrier_simplex])
    have hcarrier_conn:
      "top1_connected_on (geotop_K_carrier K x)
        (subspace_topology UNIV geotop_euclidean_topology
          (geotop_K_carrier K x))"
      by (rule top1_path_connected_on_geotop_imp_connected[OF hcarrier_path_conn])
    have hcarrier_region_union:
      "geotop_K_carrier K x \<subseteq> ?R\<^sub>1 \<union> ?R\<^sub>2"
    proof
      fix y
      assume hycarrier: "y \<in> geotop_K_carrier K x"
      have hy_not_chord: "y \<notin> closed_segment v\<^sub>0 v\<^sub>2"
        using hcarrier_no_chord hycarrier by (by100 blast)
    ...
    have hcarrier_side:
      "geotop_K_carrier K x \<subseteq> ?R\<^sub>1 \<or> geotop_K_carrier K x \<subseteq> ?R\<^sub>2"
      by (rule Theorem_GT_1_10
          [OF hTU hside_separated hcarrier_region_union hcarrier_conn])
    have hxR\<^sub>1: "x \<in> ?R\<^sub>1"
      by (rule hside1_off_chord_point_in_R\<^sub>1[OF hxB hx_not_chord])
    show "geotop_K_carrier K x \<subseteq> ?B\<^sub>1"
    proof (rule disjE[OF hcarrier_side])
      assume hsubR\<^sub>1: "geotop_K_carrier K x \<subseteq> ?R\<^sub>1"
      show ?thesis
        using hsubR\<^sub>1 hR\<^sub>1_sub_B\<^sub>1 by (by100 blast)
    next
      assume hsubR\<^sub>2: "geotop_K_carrier K x \<subseteq> ?R\<^sub>2"
      have hxR\<^sub>2: "x \<in> ?R\<^sub>2"
        using hsubR\<^sub>2 hx_carrier by (by100 blast)
    ...
    qed
  qed
  have hcarrier_side2_avoids_chord:
    "\<And>x. x \<in> ?B\<^sub>2 \<Longrightarrow> x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 = {} \<Longrightarrow>
      geotop_K_carrier K x \<subseteq> ?B\<^sub>2"
  proof -
    fix x
    ...
    proof (rule disjE[OF hcarrier_side])
      assume hsubR\<^sub>1: "geotop_K_carrier K x \<subseteq> ?R\<^sub>1"
      have hxR\<^sub>1: "x \<in> ?R\<^sub>1"
        using hsubR\<^sub>1 hx_carrier by (by100 blast)
      have "x \<in> ?R\<^sub>1 \<inter> ?R\<^sub>2"
        using hxR\<^sub>1 hxR\<^sub>2 by (by100 blast)
      thus ?thesis
        using hside_open_regions_disjoint by (by100 blast)
  ...
  have hcarrier_side1_singleton_dim2:
    "\<And>x v. x \<in> ?B\<^sub>1 \<Longrightarrow>
      x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 = {v} \<Longrightarrow>
      geotop_simplex_dim (geotop_K_carrier K x) 2 \<Longrightarrow>
      geotop_K_carrier K x \<subseteq> ?B\<^sub>1"
  proof -
    fix x v
    assume hxB: "x \<in> ?B\<^sub>1"
    ...
    proof -
      ...
      have hC_conn:
        ...
        by (rule hcarrier_chord_singleton_dim2_minus_connected[OF hdim2])
      have hC_region_union: "?C \<subseteq> ?R\<^sub>1 \<union> ?R\<^sub>2"
      proof
        fix y
        assume hyC: "y \<in> ?C"
    ...
    qed
  qed
  have hcarrier_side2_singleton_dim2:
  ...
  proof -
    ...
    qed
  qed
  have hcarrier_chord_full_minus_connected:
    "\<And>x. x \<in> ?B\<^sub>1 \<union> ?B\<^sub>2 \<Longrightarrow>
      x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 =
        closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      top1_connected_on
        (geotop_K_carrier K x - closed_segment v\<^sub>0 v\<^sub>2)
        (subspace_topology UNIV geotop_euclidean_topology
          (geotop_K_carrier K x - closed_segment v\<^sub>0 v\<^sub>2))"
  proof -
    fix x
    ...
    have hdim2:
      "geotop_simplex_dim (geotop_K_carrier K x) 2"
      by (rule hcarrier_chord_full_dim2
          [OF hxB hx_not_chord hinter_full])
    have hinter_ne:
      "geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 \<noteq> {}"
    proof -
      have "v\<^sub>0 \<in> geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2"
    ...
    show "top1_connected_on
      ...
      (subspace_topology UNIV geotop_euclidean_topology
        (geotop_K_carrier K x - closed_segment v\<^sub>0 v\<^sub>2))"
      by (rule geotop_2simplex_delete_edge_face_connected_prefix
          [OF hdim2 hchord_segment_edge hface_chord_carrier])
  qed
  have hcarrier_side1_full_chord:
    "\<And>x. x \<in> ?B\<^sub>1 \<Longrightarrow>
      x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 =
        closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<subseteq> ?B\<^sub>1"
  proof -
    ...
    show "geotop_K_carrier K x \<subseteq> ?B\<^sub>1"
    proof -
      let ?C = "geotop_K_carrier K x - closed_segment v\<^sub>0 v\<^sub>2"
      have hxB_union: "x \<in> ?B\<^sub>1 \<union> ?B\<^sub>2"
        using hxB by (by100 blast)
      have hxK: "x \<in> geotop_polyhedron K"
        using hB\<^sub>1_sub_K hxB by (by100 blast)
      have hx_carrier: "x \<in> geotop_K_carrier K x"
        by (rule geotop_K_carrier_contains_point[OF hK hK_fin hxK])
      have hxC: "x \<in> ?C"
        using hx_carrier hx_not_chord by (by100 blast)
      have hC_conn:
        "top1_connected_on ?C
          (subspace_topology UNIV geotop_euclidean_topology ?C)"
        by (rule hcarrier_chord_full_minus_connected
            [OF hxB_union hx_not_chord hinter_full])
      have hC_region_union: "?C \<subseteq> ?R\<^sub>1 \<union> ?R\<^sub>2"
      proof
        fix y
        assume hyC: "y \<in> ?C"
      ...
      proof (rule disjE[OF hC_side])
        ...
        proof
          ...
          assume hycarrier: "y \<in> geotop_K_carrier K x"
          show "y \<in> ?B\<^sub>1"
          proof (cases "y \<in> closed_segment v\<^sub>0 v\<^sub>2")
            case True
            show ?thesis
              using True hchord_segment_sub_B\<^sub>1 by (by100 blast)
          next
    ...
    qed
  qed
  have hcarrier_side2_full_chord:
    "\<And>x. x \<in> ?B\<^sub>2 \<Longrightarrow>
      x \<notin> closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 =
        closed_segment v\<^sub>0 v\<^sub>2 \<Longrightarrow>
      geotop_K_carrier K x \<subseteq> ?B\<^sub>2"
      ...
      next
        ...
        proof
          ...
          proof (cases "y \<in> closed_segment v\<^sub>0 v\<^sub>2")
            case True
            show ?thesis
              using True hchord_segment_sub_B\<^sub>2 by (by100 blast)
          next
            case False
            have "y \<in> ?C"
              using hycarrier False by (by100 blast)
  ...
  qed
  have hcarrier_side1:
    "\<forall>x\<in>?B\<^sub>1. geotop_K_carrier K x \<subseteq> ?B\<^sub>1"
  proof
    fix x
    assume hxB: "x \<in> ?B\<^sub>1"
    show "geotop_K_carrier K x \<subseteq> ?B\<^sub>1"
    proof (cases "x \<in> closed_segment v\<^sub>0 v\<^sub>2")
      case True
      show ?thesis
        by (rule hcarrier_side1_on_chord[OF hxB True])
    next
      case False
      have hx_not_chord: "x \<notin> closed_segment v\<^sub>0 v\<^sub>2"
        using False by (by100 blast)
      have hxB_union: "x \<in> ?B\<^sub>1 \<union> ?B\<^sub>2"
        using hxB by (by100 blast)
      show ?thesis
      proof (cases "geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 = {}")
        case True
        show ?thesis
          by (rule hcarrier_side1_avoids_chord
              [OF hxB hx_not_chord True])
      next
        case False
        have hinter_ne:
          "geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 \<noteq> {}"
        ...
        have hinter_cases:
          ...
          \<or> geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 =
              closed_segment v\<^sub>0 v\<^sub>2"
          by (rule hcarrier_chord_inter_cases[OF hxB_union hinter_ne])
        show ?thesis
          using hinter_cases
            hcarrier_side1_singleton[OF hxB hx_not_chord, of v\<^sub>0]
            hcarrier_side1_singleton[OF hxB hx_not_chord, of v\<^sub>2]
            hcarrier_side1_full_chord[OF hxB hx_not_chord]
          by (by100 blast)
      qed
    qed
  qed
  have hcarrier_side2:
    "\<forall>x\<in>?B\<^sub>2. geotop_K_carrier K x \<subseteq> ?B\<^sub>2"
  proof
    fix x
    assume hxB: "x \<in> ?B\<^sub>2"
    show "geotop_K_carrier K x \<subseteq> ?B\<^sub>2"
    proof (cases "x \<in> closed_segment v\<^sub>0 v\<^sub>2")
      case True
      show ?thesis
        by (rule hcarrier_side2_on_chord[OF hxB True])
    next
      case False
      have hx_not_chord: "x \<notin> closed_segment v\<^sub>0 v\<^sub>2"
        using False by (by100 blast)
      ...
      proof (cases "geotop_K_carrier K x \<inter> closed_segment v\<^sub>0 v\<^sub>2 = {}")
        case True
        show ?thesis
          by (rule hcarrier_side2_avoids_chord
              [OF hxB hx_not_chord True])
      next
        case False
        have hinter_ne:
        ...
        show ?thesis
          using hinter_cases
            hcarrier_side2_singleton[OF hxB hx_not_chord, of v\<^sub>0]
            hcarrier_side2_singleton[OF hxB hx_not_chord, of v\<^sub>2]
            hcarrier_side2_full_chord[OF hxB hx_not_chord]
          by (by100 blast)
      qed
    qed
  qed
  ...
  have h\<theta>_not_sub_B\<^sub>2: "\<not> \<theta> \<subseteq> ?B\<^sub>2"
  proof
    assume h\<theta>_sub_B\<^sub>2: "\<theta> \<subseteq> ?B\<^sub>2"
    have h\<theta>cL\<^sub>2: "\<theta>\<^sub>c \<in> L\<^sub>2"
      using h\<theta>\<^sub>c_T\<^sub>2 by (by100 blast)
    have h\<theta>c_sub_B\<^sub>2: "\<theta>\<^sub>c \<subseteq> ?B\<^sub>2"
      using hL\<^sub>2_def h\<theta>cL\<^sub>2 by (by100 blast)
    have h\<theta>_ne_\<theta>c: "\<theta> \<noteq> \<theta>\<^sub>c"
      using h\<theta>\<^sub>c_ne_\<theta> by (by100 blast)
    have hri_sub_int_union:
      "rel_interior (closed_segment v\<^sub>0 v\<^sub>2) \<subseteq> interior (\<theta> \<union> \<theta>\<^sub>c)"
      by (rule geotop_complex_two_2simplex_shared_edge_rel_interior_subset_HOL_interior_union_prefix
          [OF hK h\<theta>K h\<theta>\<^sub>cK h\<theta>2 h\<theta>\<^sub>c2 h\<theta>_ne_\<theta>c
            hchord_segment_face_\<theta> h\<theta>\<^sub>c_chord_face hchord_segment_edge])
    have h\<theta>_union_\<theta>c_sub_B\<^sub>2: "\<theta> \<union> \<theta>\<^sub>c \<subseteq> ?B\<^sub>2"
      using h\<theta>_sub_B\<^sub>2 h\<theta>c_sub_B\<^sub>2 by (by100 blast)
    have hint_union_sub_B\<^sub>2: "interior (\<theta> \<union> \<theta>\<^sub>c) \<subseteq> interior ?B\<^sub>2"
      by (rule interior_mono[OF h\<theta>_union_\<theta>c_sub_B\<^sub>2])
    have hri_sub_int_B\<^sub>2:
      "rel_interior (closed_segment v\<^sub>0 v\<^sub>2) \<subseteq> interior ?B\<^sub>2"
      using hri_sub_int_union hint_union_sub_B\<^sub>2 by (by100 blast)
    have hchord_segment_dim1: "geotop_simplex_dim (closed_segment v\<^sub>0 v\<^sub>2) 1"
      using hchord_segment_edge unfolding geotop_is_edge_def by (by100 simp)
    have hchord_segment_simplex: "geotop_is_simplex (closed_segment v\<^sub>0 v\<^sub>2)"
      by (rule geotop_simplex_dim_imp_is_simplex[OF hchord_segment_dim1])
    ...
    obtain x where hxri: "x \<in> rel_interior (closed_segment v\<^sub>0 v\<^sub>2)"
      using hri_ne by (by100 blast)
    have hxint: "x \<in> interior ?B\<^sub>2"
      using hri_sub_int_B\<^sub>2 hxri by (by100 blast)
    have "x \<in> rel_interior (closed_segment v\<^sub>0 v\<^sub>2) \<inter> interior ?B\<^sub>2"
      using hxri hxint by (by100 blast)
    thus False
      using hchord_rel_interior_disjoint_B\<^sub>2_HOL_interior by (by100 blast)
  qed
  have h\<theta>_not_T\<^sub>2: "\<theta> \<notin> ?T\<^sub>2"
  proof
    assume h\<theta>T\<^sub>2: "\<theta> \<in> ?T\<^sub>2"
    have h\<theta>L\<^sub>2: "\<theta> \<in> L\<^sub>2"
      using h\<theta>T\<^sub>2 by (by100 blast)
    have "\<theta> \<subseteq> ?B\<^sub>2"
      using hL\<^sub>2_def h\<theta>L\<^sub>2 by (by100 blast)
    thus False
      using h\<theta>_not_sub_B\<^sub>2 by (by100 blast)
  qed
  ...
  proof -
    have h\<theta>c_not_T\<^sub>1: "\<theta>\<^sub>c \<notin> ?T\<^sub>1"
      by (rule h\<theta>\<^sub>c_not_T\<^sub>1)
    show ?thesis
      using h\<theta>\<^sub>c_T h\<theta>c_not_T\<^sub>1 by (by100 blast)
  qed
  have hside_omits_T\<^sub>2: "\<exists>\<rho>\<in>?T. \<rho> \<notin> ?T\<^sub>2"
  proof -
    show ?thesis
      using h\<theta>_T h\<theta>_not_T\<^sub>2 by (by100 blast)
  qed
  ...
  proof -
    assume hex:
      "\<exists>\<sigma> \<tau>. \<sigma> \<in> K\<^sub>1 \<and> geotop_simplex_dim \<sigma> 2 \<and>
        ...
        \<sigma> \<noteq> \<tau>"
    obtain \<sigma> \<tau>
      where h\<sigma>K\<^sub>1: "\<sigma> \<in> K\<^sub>1"
        and h\<sigma>2: "geotop_simplex_dim \<sigma> 2"
        and h\<sigma>E_card_le2:
          "card {e\<in>K\<^sub>1. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J} \<le> 2"
        and h\<sigma>contact:
  ...
  have hderived_contact_other_segment_off_base_pre:
    "\<exists>x. x \<in> \<theta> \<inter> J
      ...
      \<and> x \<in> (closed_segment v\<^sub>0 v\<^sub>2 - {v\<^sub>0})
          \<union> (closed_segment v\<^sub>1 v\<^sub>2 - {v\<^sub>1})"
    by (rule geotop_nonfree_boundary_triangle_contact_on_nonbase_segment_off_base_prefix
        [OF hJ hK hK_poly hT_gt1_pre h\<theta>K h\<theta>2 h\<theta>_vertices
          hv\<^sub>0v\<^sub>1 hv\<^sub>2_not hv\<^sub>0v\<^sub>1_sub_J h\<theta>_not_free])
  obtain x where hx\<theta>J: "x \<in> \<theta> \<inter> J"
    and hx_not_base: "x \<notin> closed_segment v\<^sub>0 v\<^sub>1"
    and hx_nonbase:
                  ...
                  card {e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<tau> \<and> e \<subseteq> J'} \<le> 2 \<and>
                    ...
                    proof -
                      ...
                      have hJ\<^sub>2_boundary_def:
                        ...
                        using hsubdisk_book by (by100 blast)
                      have hC\<^sub>1_sub_J': "C\<^sub>1 \<subseteq> J'"
                        using hJ'_boundary_split by (by100 blast)
                      have hC\<^sub>2_sub_J': "C\<^sub>2 \<subseteq> J'"
                        using hJ'_boundary_split by (by100 blast)
                      have hC\<^sub>1_sub_J\<^sub>1: "C\<^sub>1 \<subseteq> J\<^sub>1"
                        using hJ\<^sub>1_boundary_def by (by100 blast)
                      have hC\<^sub>2_sub_J\<^sub>2: "C\<^sub>2 \<subseteq> J\<^sub>2"
                        using hJ\<^sub>2_boundary_def by (by100 blast)
                      have hchord_sub_J\<^sub>1: "closed_segment v\<^sub>0 v\<^sub>2 \<subseteq> J\<^sub>1"
                        using hJ\<^sub>1_boundary_def by (by100 blast)
                      have hchord_sub_J\<^sub>2: "closed_segment v\<^sub>0 v\<^sub>2 \<subseteq> J\<^sub>2"
                        using hJ\<^sub>2_boundary_def by (by100 blast)
                      ...
                      have hL\<^sub>1_selected_edges_fin:
                        ...
                        using hL\<^sub>1_fin by (by100 simp)
                      have hL\<^sub>2_selected_edges_fin:
                        "\<And>\<sigma>. finite
                          {e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'}"
                        using hL\<^sub>2_fin by (by100 simp)
                      have hL\<^sub>1_canonical_selected_edges_transfer_to_parent:
                        "\<And>\<sigma>. \<sigma> \<in> L\<^sub>1 \<Longrightarrow> geotop_simplex_dim \<sigma> 2 \<Longrightarrow>
                          finite {e\<in>L\<^sub>1. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<Longrightarrow>
                        ...
                        by (rule geotop_free_2_simplex_selected_edge_set_card_le2_transfer_prefix
                            [OF hL\<^sub>1_sub_K])
                      have hL\<^sub>2_canonical_selected_edges_transfer_to_parent:
                        "\<And>\<sigma>. \<sigma> \<in> L\<^sub>2 \<Longrightarrow> geotop_simplex_dim \<sigma> 2 \<Longrightarrow>
                          finite {e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<Longrightarrow>
                          card {e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<le> 2 \<Longrightarrow>
                          \<sigma> \<inter> J' =
                            \<Union>{e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<Longrightarrow>
                          geotop_free_2_simplex K J' \<sigma>"
                        by (rule geotop_free_2_simplex_selected_edge_set_card_le2_transfer_prefix
                            [OF hL\<^sub>2_sub_K])
                      have hL\<^sub>1_canonical_selected_edges_card_le2_transfer_to_parent:
                        "\<And>\<sigma>. \<sigma> \<in> L\<^sub>1 \<Longrightarrow> geotop_simplex_dim \<sigma> 2 \<Longrightarrow>
                          card {e\<in>L\<^sub>1. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<le> 2 \<Longrightarrow>
                          \<sigma> \<inter> J' =
                            \<Union>{e\<in>L\<^sub>1. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<Longrightarrow>
                      ...
                      proof -
                        ...
                        have hE_fin:
                          "finite {e\<in>L\<^sub>1. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'}"
                          by (rule hL\<^sub>1_selected_edges_fin)
                        show "geotop_free_2_simplex K J' \<sigma>"
                          by (rule hL\<^sub>1_canonical_selected_edges_transfer_to_parent
                              [OF h\<sigma>L\<^sub>1 h\<sigma>2 hE_fin hE_card_le2 hcontact])
                      qed
                      have hL\<^sub>2_canonical_selected_edges_card_le2_transfer_to_parent:
                        "\<And>\<sigma>. \<sigma> \<in> L\<^sub>2 \<Longrightarrow> geotop_simplex_dim \<sigma> 2 \<Longrightarrow>
                          card {e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<le> 2 \<Longrightarrow>
                          \<sigma> \<inter> J' =
                            \<Union>{e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<Longrightarrow>
                      ...
                      proof -
                        fix \<sigma>
                        assume h\<sigma>L\<^sub>2: "\<sigma> \<in> L\<^sub>2"
                        assume h\<sigma>2: "geotop_simplex_dim \<sigma> 2"
                        assume hE_card_le2:
                          "card {e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'} \<le> 2"
                        assume hcontact:
                          "\<sigma> \<inter> J' =
                            \<Union>{e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'}"
                        have hE_fin:
                          "finite {e\<in>L\<^sub>2. geotop_is_edge e \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J'}"
                          by (rule hL\<^sub>2_selected_edges_fin)
                        show "geotop_free_2_simplex K J' \<sigma>"
                          by (rule hL\<^sub>2_canonical_selected_edges_transfer_to_parent
                              [OF h\<sigma>L\<^sub>2 h\<sigma>2 hE_fin hE_card_le2 hcontact])
                      qed
                      have hparent_free_pair_finishes:
                        "\<And>\<sigma> \<tau>. \<sigma> \<in> L\<^sub>1 \<Longrightarrow> \<tau> \<in> L\<^sub>2 \<Longrightarrow> \<sigma> \<noteq> \<tau> \<Longrightarrow>
                          geotop_free_2_simplex K J' \<sigma> \<Longrightarrow>
                          geotop_free_2_simplex K J' \<tau> \<Longrightarrow>
                          card ?F \<ge> 2"
                      proof -
                      ...
                      proof -
                        ...
                        qed
                        have hside_two_canonical_boundary_witnesses_each:
                          "\<exists>\<sigma>\<^sub>1 \<sigma>\<^sub>2 \<tau>\<^sub>1 \<tau>\<^sub>2.
                            \<sigma>\<^sub>1 \<in> L\<^sub>1
                            \<and> geotop_simplex_dim \<sigma>\<^sub>1 2
                            \<and> card {e\<in>L\<^sub>1. geotop_is_edge e
                              \<and> geotop_is_face e \<sigma>\<^sub>1 \<and> e \<subseteq> J\<^sub>1} \<le> 2
                            \<and> \<sigma>\<^sub>1 \<inter> J\<^sub>1 =
                              \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                \<and> geotop_is_face e \<sigma>\<^sub>1 \<and> e \<subseteq> J\<^sub>1}
                            \<and> \<sigma>\<^sub>2 \<in> L\<^sub>1
                            \<and> geotop_simplex_dim \<sigma>\<^sub>2 2
                            \<and> card {e\<in>L\<^sub>1. geotop_is_edge e
                              \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1} \<le> 2
                            \<and> \<sigma>\<^sub>2 \<inter> J\<^sub>1 =
                              \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1}
                            \<and> \<sigma>\<^sub>1 \<noteq> \<sigma>\<^sub>2
                            \<and> \<tau>\<^sub>1 \<in> L\<^sub>2
                            \<and> geotop_simplex_dim \<tau>\<^sub>1 2
                            \<and> card {e\<in>L\<^sub>2. geotop_is_edge e
                              \<and> geotop_is_face e \<tau>\<^sub>1 \<and> e \<subseteq> J\<^sub>2} \<le> 2
                            \<and> \<tau>\<^sub>1 \<inter> J\<^sub>2 =
                              \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                \<and> geotop_is_face e \<tau>\<^sub>1 \<and> e \<subseteq> J\<^sub>2}
                            \<and> \<tau>\<^sub>2 \<in> L\<^sub>2
                            \<and> geotop_simplex_dim \<tau>\<^sub>2 2
                            \<and> card {e\<in>L\<^sub>2. geotop_is_edge e
                              \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2} \<le> 2
                            \<and> \<tau>\<^sub>2 \<inter> J\<^sub>2 =
                              \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2}
                            \<and> \<tau>\<^sub>1 \<noteq> \<tau>\<^sub>2"
                          (**
                            Canonicalize both free choices on each side.  This
                            preserves the two-candidate data needed to discard
                            a possible chord-only witness later. **)
                        proof -
                          obtain \<sigma>\<^sub>1 \<sigma>\<^sub>2 \<tau>\<^sub>1 \<tau>\<^sub>2
                            where h\<sigma>\<^sub>1L\<^sub>1: "\<sigma>\<^sub>1 \<in> L\<^sub>1"
                              and h\<sigma>\<^sub>1free:
                                "geotop_free_2_simplex L\<^sub>1 J\<^sub>1 \<sigma>\<^sub>1"
                              and h\<sigma>\<^sub>2L\<^sub>1: "\<sigma>\<^sub>2 \<in> L\<^sub>1"
                              and h\<sigma>\<^sub>2free:
                                "geotop_free_2_simplex L\<^sub>1 J\<^sub>1 \<sigma>\<^sub>2"
                              and h\<sigma>\<^sub>1_ne_\<sigma>\<^sub>2: "\<sigma>\<^sub>1 \<noteq> \<sigma>\<^sub>2"
                              and h\<tau>\<^sub>1L\<^sub>2: "\<tau>\<^sub>1 \<in> L\<^sub>2"
                              and h\<tau>\<^sub>1free:
                                "geotop_free_2_simplex L\<^sub>2 J\<^sub>2 \<tau>\<^sub>1"
                              and h\<tau>\<^sub>2L\<^sub>2: "\<tau>\<^sub>2 \<in> L\<^sub>2"
                              and h\<tau>\<^sub>2free:
                                "geotop_free_2_simplex L\<^sub>2 J\<^sub>2 \<tau>\<^sub>2"
                              and h\<tau>\<^sub>1_ne_\<tau>\<^sub>2: "\<tau>\<^sub>1 \<noteq> \<tau>\<^sub>2"
                            using hside_free_distinct_pairs_from_counts
                            by (elim exE conjE)
                          have h\<sigma>\<^sub>12: "geotop_simplex_dim \<sigma>\<^sub>1 2"
                            using h\<sigma>\<^sub>1free unfolding geotop_free_2_simplex_def
                            by (by100 blast)
                          have h\<sigma>\<^sub>22: "geotop_simplex_dim \<sigma>\<^sub>2 2"
                            using h\<sigma>\<^sub>2free unfolding geotop_free_2_simplex_def
                            by (by100 blast)
                          have h\<tau>\<^sub>12: "geotop_simplex_dim \<tau>\<^sub>1 2"
                            using h\<tau>\<^sub>1free unfolding geotop_free_2_simplex_def
                            by (by100 blast)
                          have h\<tau>\<^sub>22: "geotop_simplex_dim \<tau>\<^sub>2 2"
                            using h\<tau>\<^sub>2free unfolding geotop_free_2_simplex_def
                            by (by100 blast)
                          have h\<sigma>\<^sub>1_can:
                            ...
                            by (rule geotop_free_2_simplex_canonical_selected_edge_contact_prefix
                                [OF hJ\<^sub>1 hL\<^sub>1_complex hL\<^sub>1_poly_eq h\<sigma>\<^sub>1L\<^sub>1
                                  h\<sigma>\<^sub>12 hT\<^sub>1_gt1 h\<sigma>\<^sub>1free])
                          have h\<sigma>\<^sub>2_can:
                            "finite {e\<in>L\<^sub>1. geotop_is_edge e
                                \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1}
                              \<and> card {e\<in>L\<^sub>1. geotop_is_edge e
                                \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1} \<le> 2
                              \<and> \<sigma>\<^sub>2 \<inter> J\<^sub>1 =
                                \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                  \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1}"
                            by (rule geotop_free_2_simplex_canonical_selected_edge_contact_prefix
                                [OF hJ\<^sub>1 hL\<^sub>1_complex hL\<^sub>1_poly_eq h\<sigma>\<^sub>2L\<^sub>1
                                  h\<sigma>\<^sub>22 hT\<^sub>1_gt1 h\<sigma>\<^sub>2free])
                          have h\<tau>\<^sub>1_can:
                            "finite {e\<in>L\<^sub>2. geotop_is_edge e
                              ...
                              \<and> \<tau>\<^sub>1 \<inter> J\<^sub>2 =
                                \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>\<^sub>1 \<and> e \<subseteq> J\<^sub>2}"
                            by (rule geotop_free_2_simplex_canonical_selected_edge_contact_prefix
                                [OF hJ\<^sub>2 hL\<^sub>2_complex hL\<^sub>2_poly_eq h\<tau>\<^sub>1L\<^sub>2
                                  h\<tau>\<^sub>12 hT\<^sub>2_gt1 h\<tau>\<^sub>1free])
                          have h\<tau>\<^sub>2_can:
                            "finite {e\<in>L\<^sub>2. geotop_is_edge e
                                \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2}
                              \<and> card {e\<in>L\<^sub>2. geotop_is_edge e
                                \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2} \<le> 2
                              \<and> \<tau>\<^sub>2 \<inter> J\<^sub>2 =
                                \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2}"
                            by (rule geotop_free_2_simplex_canonical_selected_edge_contact_prefix
                                [OF hJ\<^sub>2 hL\<^sub>2_complex hL\<^sub>2_poly_eq h\<tau>\<^sub>2L\<^sub>2
                                  h\<tau>\<^sub>22 hT\<^sub>2_gt1 h\<tau>\<^sub>2free])
                          have h\<sigma>\<^sub>1_card:
                            "card {e\<in>L\<^sub>1. geotop_is_edge e
                              \<and> geotop_is_face e \<sigma>\<^sub>1 \<and> e \<subseteq> J\<^sub>1} \<le> 2"
                            using h\<sigma>\<^sub>1_can by (by100 blast)
                          have h\<sigma>\<^sub>1_contact:
                            "\<sigma>\<^sub>1 \<inter> J\<^sub>1 =
                              \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                \<and> geotop_is_face e \<sigma>\<^sub>1 \<and> e \<subseteq> J\<^sub>1}"
                            using h\<sigma>\<^sub>1_can by (by100 blast)
                          have h\<sigma>\<^sub>2_card:
                            "card {e\<in>L\<^sub>1. geotop_is_edge e
                              \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1} \<le> 2"
                            using h\<sigma>\<^sub>2_can by (by100 blast)
                          have h\<sigma>\<^sub>2_contact:
                            "\<sigma>\<^sub>2 \<inter> J\<^sub>1 =
                              \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1}"
                            using h\<sigma>\<^sub>2_can by (by100 blast)
                          have h\<tau>\<^sub>1_card:
                            "card {e\<in>L\<^sub>2. geotop_is_edge e
                              \<and> geotop_is_face e \<tau>\<^sub>1 \<and> e \<subseteq> J\<^sub>2} \<le> 2"
                            using h\<tau>\<^sub>1_can by (by100 blast)
                          have h\<tau>\<^sub>1_contact:
                            "\<tau>\<^sub>1 \<inter> J\<^sub>2 =
                              \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                \<and> geotop_is_face e \<tau>\<^sub>1 \<and> e \<subseteq> J\<^sub>2}"
                            using h\<tau>\<^sub>1_can by (by100 blast)
                          have h\<tau>\<^sub>2_card:
                            "card {e\<in>L\<^sub>2. geotop_is_edge e
                              \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2} \<le> 2"
                            using h\<tau>\<^sub>2_can by (by100 blast)
                          have h\<tau>\<^sub>2_contact:
                            "\<tau>\<^sub>2 \<inter> J\<^sub>2 =
                              \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2}"
                            using h\<tau>\<^sub>2_can by (by100 blast)
                          show ?thesis
                            ...
                            apply (rule exI[where x = "\<tau>\<^sub>1"])
                            apply (rule exI[where x = "\<tau>\<^sub>2"])
                            using h\<sigma>\<^sub>1L\<^sub>1 h\<sigma>\<^sub>12 h\<sigma>\<^sub>1_card h\<sigma>\<^sub>1_contact
                              h\<sigma>\<^sub>2L\<^sub>1 h\<sigma>\<^sub>22 h\<sigma>\<^sub>2_card h\<sigma>\<^sub>2_contact h\<sigma>\<^sub>1_ne_\<sigma>\<^sub>2
                              h\<tau>\<^sub>1L\<^sub>2 h\<tau>\<^sub>12 h\<tau>\<^sub>1_card h\<tau>\<^sub>1_contact
                              h\<tau>\<^sub>2L\<^sub>2 h\<tau>\<^sub>22 h\<tau>\<^sub>2_card h\<tau>\<^sub>2_contact h\<tau>\<^sub>1_ne_\<tau>\<^sub>2
                            apply (intro conjI)
                            apply assumption+
                            done
                        qed
                        have hside_two_canonical_one_avoids_\<theta>_each:
                          "\<exists>\<sigma> \<sigma>' \<tau> \<tau>'. \<sigma> \<in> L\<^sub>1
                            ...
                            \<and> \<tau> \<noteq> \<tau>'"
                          (**
                            Keep the two canonical side candidates, but orient
                            each pair so the first candidate is not the cut
                            triangle.  This prepares the later chord-only
                            filtering argument without losing the spare
                            candidate on either side. **)
                        proof -
                          obtain \<sigma>\<^sub>1 \<sigma>\<^sub>2 \<tau>\<^sub>1 \<tau>\<^sub>2
                            where h\<sigma>\<^sub>1L\<^sub>1: "\<sigma>\<^sub>1 \<in> L\<^sub>1"
                              and h\<sigma>\<^sub>12: "geotop_simplex_dim \<sigma>\<^sub>1 2"
                              and h\<sigma>\<^sub>1_card:
                                "card {e\<in>L\<^sub>1. geotop_is_edge e
                                  \<and> geotop_is_face e \<sigma>\<^sub>1 \<and> e \<subseteq> J\<^sub>1} \<le> 2"
                              and h\<sigma>\<^sub>1_contact:
                                "\<sigma>\<^sub>1 \<inter> J\<^sub>1 =
                                  \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                    \<and> geotop_is_face e \<sigma>\<^sub>1 \<and> e \<subseteq> J\<^sub>1}"
                              and h\<sigma>\<^sub>2L\<^sub>1: "\<sigma>\<^sub>2 \<in> L\<^sub>1"
                              and h\<sigma>\<^sub>22: "geotop_simplex_dim \<sigma>\<^sub>2 2"
                              and h\<sigma>\<^sub>2_card:
                                "card {e\<in>L\<^sub>1. geotop_is_edge e
                                  \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1} \<le> 2"
                              and h\<sigma>\<^sub>2_contact:
                                "\<sigma>\<^sub>2 \<inter> J\<^sub>1 =
                                  \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                    \<and> geotop_is_face e \<sigma>\<^sub>2 \<and> e \<subseteq> J\<^sub>1}"
                              and h\<sigma>\<^sub>1_ne_\<sigma>\<^sub>2: "\<sigma>\<^sub>1 \<noteq> \<sigma>\<^sub>2"
                              and h\<tau>\<^sub>1L\<^sub>2: "\<tau>\<^sub>1 \<in> L\<^sub>2"
                              and h\<tau>\<^sub>12: "geotop_simplex_dim \<tau>\<^sub>1 2"
                              and h\<tau>\<^sub>1_card:
                                "card {e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>\<^sub>1 \<and> e \<subseteq> J\<^sub>2} \<le> 2"
                              and h\<tau>\<^sub>1_contact:
                                "\<tau>\<^sub>1 \<inter> J\<^sub>2 =
                                  \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                    \<and> geotop_is_face e \<tau>\<^sub>1 \<and> e \<subseteq> J\<^sub>2}"
                              and h\<tau>\<^sub>2L\<^sub>2: "\<tau>\<^sub>2 \<in> L\<^sub>2"
                              and h\<tau>\<^sub>22: "geotop_simplex_dim \<tau>\<^sub>2 2"
                              and h\<tau>\<^sub>2_card:
                                "card {e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2} \<le> 2"
                              and h\<tau>\<^sub>2_contact:
                                "\<tau>\<^sub>2 \<inter> J\<^sub>2 =
                                  \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                    \<and> geotop_is_face e \<tau>\<^sub>2 \<and> e \<subseteq> J\<^sub>2}"
                              and h\<tau>\<^sub>1_ne_\<tau>\<^sub>2: "\<tau>\<^sub>1 \<noteq> \<tau>\<^sub>2"
                            using hside_two_canonical_boundary_witnesses_each
                            by (elim exE conjE)
                          have hside1_oriented:
                            "\<exists>\<sigma> \<sigma>'. \<sigma> \<in> L\<^sub>1
                              \<and> geotop_simplex_dim \<sigma> 2
                              \<and> \<sigma> \<noteq> \<theta>
                              \<and> card {e\<in>L\<^sub>1. geotop_is_edge e
                                \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J\<^sub>1} \<le> 2
                          ...
                          proof (cases "\<sigma>\<^sub>1 = \<theta>")
                            case True
                            have h\<sigma>\<^sub>2_ne_\<theta>: "\<sigma>\<^sub>2 \<noteq> \<theta>"
                              using True h\<sigma>\<^sub>1_ne_\<sigma>\<^sub>2 by (by100 blast)
                            show ?thesis
                              apply (rule exI[where x = "\<sigma>\<^sub>2"])
                              apply (rule exI[where x = "\<sigma>\<^sub>1"])
                              using h\<sigma>\<^sub>2L\<^sub>1 h\<sigma>\<^sub>22 h\<sigma>\<^sub>2_ne_\<theta> h\<sigma>\<^sub>2_card
                                h\<sigma>\<^sub>2_contact h\<sigma>\<^sub>1L\<^sub>1 h\<sigma>\<^sub>12 h\<sigma>\<^sub>1_card
                                h\<sigma>\<^sub>1_contact h\<sigma>\<^sub>1_ne_\<sigma>\<^sub>2
                              apply (intro conjI)
                              apply assumption+
                              apply (by100 blast)
                              done
                          next
                            ...
                            show ?thesis
                              apply (rule exI[where x = "\<sigma>\<^sub>1"])
                              apply (rule exI[where x = "\<sigma>\<^sub>2"])
                              using h\<sigma>\<^sub>1L\<^sub>1 h\<sigma>\<^sub>12 False h\<sigma>\<^sub>1_card h\<sigma>\<^sub>1_contact
                                h\<sigma>\<^sub>2L\<^sub>1 h\<sigma>\<^sub>22 h\<sigma>\<^sub>2_card h\<sigma>\<^sub>2_contact
                                h\<sigma>\<^sub>1_ne_\<sigma>\<^sub>2
                              apply (intro conjI)
                              apply assumption+
                              done
                          qed
                          have hside2_oriented:
                            "\<exists>\<tau> \<tau>'. \<tau> \<in> L\<^sub>2
                              \<and> geotop_simplex_dim \<tau> 2
                              \<and> \<tau> \<noteq> \<theta>
                              \<and> card {e\<in>L\<^sub>2. geotop_is_edge e
                                \<and> geotop_is_face e \<tau> \<and> e \<subseteq> J\<^sub>2} \<le> 2
                              ...
                              \<and> \<tau>' \<inter> J\<^sub>2 =
                                \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>' \<and> e \<subseteq> J\<^sub>2}
                              \<and> \<tau> \<noteq> \<tau>'"
                          proof (cases "\<tau>\<^sub>1 = \<theta>")
                            case True
                            have h\<tau>\<^sub>2_ne_\<theta>: "\<tau>\<^sub>2 \<noteq> \<theta>"
                              using True h\<tau>\<^sub>1_ne_\<tau>\<^sub>2 by (by100 blast)
                            show ?thesis
                              apply (rule exI[where x = "\<tau>\<^sub>2"])
                              apply (rule exI[where x = "\<tau>\<^sub>1"])
                              using h\<tau>\<^sub>2L\<^sub>2 h\<tau>\<^sub>22 h\<tau>\<^sub>2_ne_\<theta> h\<tau>\<^sub>2_card
                                h\<tau>\<^sub>2_contact h\<tau>\<^sub>1L\<^sub>2 h\<tau>\<^sub>12 h\<tau>\<^sub>1_card
                                h\<tau>\<^sub>1_contact h\<tau>\<^sub>1_ne_\<tau>\<^sub>2
                              apply (intro conjI)
                              apply assumption+
                              apply (by100 blast)
                              done
                          next
                            ...
                            show ?thesis
                              apply (rule exI[where x = "\<tau>\<^sub>1"])
                              apply (rule exI[where x = "\<tau>\<^sub>2"])
                              using h\<tau>\<^sub>1L\<^sub>2 h\<tau>\<^sub>12 False h\<tau>\<^sub>1_card h\<tau>\<^sub>1_contact
                                h\<tau>\<^sub>2L\<^sub>2 h\<tau>\<^sub>22 h\<tau>\<^sub>2_card h\<tau>\<^sub>2_contact
                                h\<tau>\<^sub>1_ne_\<tau>\<^sub>2
                              apply (intro conjI)
                              apply assumption+
                              done
                          qed
                          obtain \<sigma> \<sigma>' where h\<sigma>L\<^sub>1: "\<sigma> \<in> L\<^sub>1"
                            and h\<sigma>2: "geotop_simplex_dim \<sigma> 2"
                            ...
                            using hside1_oriented by (elim exE conjE)
                          obtain \<tau> \<tau>' where h\<tau>L\<^sub>2: "\<tau> \<in> L\<^sub>2"
                            and h\<tau>2: "geotop_simplex_dim \<tau> 2"
                            ...
                            using hside2_oriented by (elim exE conjE)
                          show ?thesis
                        ...
                        qed
                        have hside_oriented_primary_witnesses_distinct:
                          "\<exists>\<sigma> \<sigma>' \<tau> \<tau>'. \<sigma> \<in> L\<^sub>1
                            \<and> geotop_simplex_dim \<sigma> 2
                            \<and> \<sigma> \<noteq> \<theta>
                            \<and> card {e\<in>L\<^sub>1. geotop_is_edge e
                            ...
                            \<and> \<sigma> \<noteq> \<tau>"
                          (**
                            The oriented primary side witnesses live in the
                            disjoint side 2-simplex sets.  Recording this now
                            preserves the distinctness needed after the later
                            parent-boundary chord filter chooses the primary
                            witnesses. **)
                        proof -
                              ...
                              and h\<tau>'_contact:
                                "\<tau>' \<inter> J\<^sub>2 =
                                  \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                    \<and> geotop_is_face e \<tau>' \<and> e \<subseteq> J\<^sub>2}"
                              and h\<tau>_ne_\<tau>': "\<tau> \<noteq> \<tau>'"
                            using hside_two_canonical_one_avoids_\<theta>_each
                            by (elim exE conjE)
                          ...
                          have h\<sigma>_ne_\<tau>: "\<sigma> \<noteq> \<tau>"
                            using hT\<^sub>1_T\<^sub>2_disjoint_book h\<sigma>T\<^sub>1 h\<tau>T\<^sub>2
                            by (by100 blast)
                          show ?thesis
                            apply (rule exI[where x = \<sigma>])
                            apply (rule exI[where x = "\<sigma>'"])
                        ...
                        proof -
                          ...
                          qed
                          have hside_free_witnesses_with_contact_cards:
                            "\<exists>\<sigma>\<^sub>L \<tau>\<^sub>L E\<^sub>\<sigma> E\<^sub>\<tau>. \<sigma>\<^sub>L \<in> L\<^sub>1
                              \<and> geotop_simplex_dim \<sigma>\<^sub>L 2
                              \<and> \<sigma>\<^sub>L \<noteq> \<theta>
                              \<and> E\<^sub>\<sigma> \<subseteq> L\<^sub>1
                              \<and> card E\<^sub>\<sigma> \<le> 2
                              \<and> \<sigma>\<^sub>L \<inter> J\<^sub>1 = \<Union>E\<^sub>\<sigma>
                              \<and> \<tau>\<^sub>L \<in> L\<^sub>2
                              \<and> geotop_simplex_dim \<tau>\<^sub>L 2
                              \<and> \<tau>\<^sub>L \<noteq> \<theta>
                              \<and> E\<^sub>\<tau> \<subseteq> L\<^sub>2
                              \<and> card E\<^sub>\<tau> \<le> 2
                              \<and> \<tau>\<^sub>L \<inter> J\<^sub>2 = \<Union>E\<^sub>\<tau>"
                          proof -
                            ...
                            have hE\<^sub>\<sigma>card: "card E\<^sub>\<sigma> \<le> 2"
                              using hE\<^sub>\<sigma>allowed
                              apply (elim disjE exE conjE)
                              apply (by100 simp)
                              apply (by100 simp)
                              apply (by100 simp)
                              done
                            have hE\<^sub>\<tau>card: "card E\<^sub>\<tau> \<le> 2"
                              using hE\<^sub>\<tau>allowed
                              apply (elim disjE exE conjE)
                              apply (by100 simp)
                              apply (by100 simp)
                              apply (by100 simp)
                            ...
                            show ?thesis
                              ...
                              apply (rule exI[where x = "E\<^sub>\<tau>"])
                              using h\<sigma>\<^sub>LL\<^sub>1 h\<sigma>\<^sub>L2 h\<sigma>\<^sub>L_ne_\<theta> hE\<^sub>\<sigma>sub
                                hE\<^sub>\<sigma>card h\<sigma>\<^sub>L_contact h\<tau>\<^sub>LL\<^sub>2 h\<tau>\<^sub>L2
                                h\<tau>\<^sub>L_ne_\<theta> hE\<^sub>\<tau>sub hE\<^sub>\<tau>card h\<tau>\<^sub>L_contact
                              apply (intro conjI)
                              apply assumption+
                              done
                          qed
                          ...
                          proof -
                              ...
                              "finite {e\<in>L\<^sub>1. geotop_is_edge e
                                ...
                                \<and> \<sigma>\<^sub>L \<inter> J\<^sub>1 =
                                  \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                    \<and> geotop_is_face e \<sigma>\<^sub>L \<and> e \<subseteq> J\<^sub>1}"
                              by (rule geotop_free_2_simplex_canonical_selected_edge_contact_prefix
                                  [OF hJ\<^sub>1 hL\<^sub>1_complex hL\<^sub>1_poly_eq h\<sigma>\<^sub>LL\<^sub>1
                                    h\<sigma>\<^sub>L2 hT\<^sub>1_gt1 h\<sigma>\<^sub>Lfree])
                            have h\<tau>\<^sub>L_can:
                              "finite {e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>\<^sub>L \<and> e \<subseteq> J\<^sub>2}
                                \<and> card {e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>\<^sub>L \<and> e \<subseteq> J\<^sub>2} \<le> 2
                                \<and> \<tau>\<^sub>L \<inter> J\<^sub>2 =
                                  \<Union>{e\<in>L\<^sub>2. geotop_is_edge e
                                    \<and> geotop_is_face e \<tau>\<^sub>L \<and> e \<subseteq> J\<^sub>2}"
                              by (rule geotop_free_2_simplex_canonical_selected_edge_contact_prefix
                                  [OF hJ\<^sub>2 hL\<^sub>2_complex hL\<^sub>2_poly_eq h\<tau>\<^sub>LL\<^sub>2
                                    h\<tau>\<^sub>L2 hT\<^sub>2_gt1 h\<tau>\<^sub>Lfree])
                            have h\<sigma>\<^sub>L_card:
                              "card {e\<in>L\<^sub>1. geotop_is_edge e
                                  \<and> geotop_is_face e \<sigma>\<^sub>L \<and> e \<subseteq> J\<^sub>1} \<le> 2"
                              using h\<sigma>\<^sub>L_can by (by100 blast)
                            have h\<sigma>\<^sub>L_contact_can:
                              "\<sigma>\<^sub>L \<inter> J\<^sub>1 =
                                \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                  \<and> geotop_is_face e \<sigma>\<^sub>L \<and> e \<subseteq> J\<^sub>1}"
                              using h\<sigma>\<^sub>L_can by (by100 blast)
                            have h\<tau>\<^sub>L_card:
                              "card {e\<in>L\<^sub>2. geotop_is_edge e
                                  \<and> geotop_is_face e \<tau>\<^sub>L \<and> e \<subseteq> J\<^sub>2} \<le> 2"
                              using h\<tau>\<^sub>L_can by (by100 blast)
                            ...
                            show ?thesis
                              ...
                              using h\<sigma>\<^sub>LL\<^sub>1 h\<sigma>\<^sub>L2 h\<sigma>\<^sub>L_ne_\<theta> h\<sigma>\<^sub>L_card
                                ...
                                h\<tau>\<^sub>L_card h\<tau>\<^sub>L_contact_can
                              apply (intro conjI)
                              apply assumption+
                              done
                          qed
                                ...
                                "finite {e\<in>L\<^sub>1. geotop_is_edge e
                                  ...
                                  \<and> \<sigma> \<inter> J\<^sub>1 =
                                    \<Union>{e\<in>L\<^sub>1. geotop_is_edge e
                                      \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J\<^sub>1}"
                                by (rule geotop_free_2_simplex_canonical_selected_edge_contact_prefix
                                    [OF hJ\<^sub>1 hL\<^sub>1_complex hL\<^sub>1_poly_eq h\<sigma>L\<^sub>1
                                      h\<sigma>2 hT\<^sub>1_gt1 h\<sigma>free])
                              have h\<sigma>card:
                                "card {e\<in>L\<^sub>1. geotop_is_edge e
                                  \<and> geotop_is_face e \<sigma> \<and> e \<subseteq> J\<^sub>1} \<le> 2"
