...
lemma geotop_arc_not_subset_singleton_prefix:
  fixes A :: "(real^2) set"
  assumes hA: "geotop_is_arc A (subspace_topology UNIV geotop_euclidean_topology A)"
  shows "\<And>x. \<not> A \<subseteq> {x}"
proof
  fix x
  assume hsub: "A \<subseteq> {x}"
  obtain \<gamma> :: "real \<Rightarrow> real^2" where h\<gamma>_arc: "arc \<gamma>"
    and h\<gamma>_img: "path_image \<gamma> = A"
    using geotop_is_arc_imp_HOL_arc[OF hA] by (by100 blast)
  have h0_img: "\<gamma> 0 \<in> path_image \<gamma>"
    unfolding path_image_def by (rule image_eqI[where x = 0], simp_all)
  have h0A: "\<gamma> 0 \<in> A"
...
lemma geotop_connected_complex_nondegenerate_subset_vertex_incident_edge_prefix:
  fixes K :: "(real^2) set set" and A :: "(real^2) set"
    and p :: "real^2"
  assumes hK: "geotop_is_complex K"
  assumes hpoly_connected: "connected (geotop_polyhedron K)"
  assumes hA_sub: "A \<subseteq> geotop_polyhedron K"
  assumes hA_not_singleton: "\<And>x. \<not> A \<subseteq> {x}"
  assumes hpK: "{p} \<in> K"
  shows "\<exists>e\<in>K. geotop_is_edge e \<and> p \<in> e"
  (**
    Connected carrier no-isolated-vertex step used in Moise 4.4: a connected
    complex carrier containing a nondegenerate arc cannot have a vertex isolated
    from all edges, since that singleton would be a clopen component. **)
proof (rule ccontr)
  assume hno: "\<not> (\<exists>e\<in>K. geotop_is_edge e \<and> p \<in> e)"
  ...
  have hsingle_top:
    ...
    by (rule geotop_complex_no_incident_edge_vertex_open_singleton_prefix
        [OF hK hp_vertex hno])
  obtain U where hsingle_eq: "{p} = geotop_polyhedron K \<inter> U"
  ...
  have hU_open: "open U"
    ...
    by (by100 simp)
  have hsingle_openin:
      "openin (top_of_set (geotop_polyhedron K)) {p}"
  ...
  have hsingle_closedin:
      "closedin (top_of_set (geotop_polyhedron K)) {p}"
  proof -
    have hclosed_single: "closed {p}"
      by (by100 simp)
    have hsingle_eq_poly:
        "{p} = geotop_polyhedron K \<inter> {p}"
  ...
  qed
  have hsingle_cases:
      "{p} = {} \<or> {p} = geotop_polyhedron K"
    using connected_clopen[THEN iffD1, OF hpoly_connected]
      hsingle_openin hsingle_closedin by (by100 blast)
  have hpoly_single: "geotop_polyhedron K = {p}"
    using hsingle_cases by (by100 blast)
  have hA_sub_single: "A \<subseteq> {p}"
    using hA_sub hpoly_single by (by100 simp)
  show False
    using hA_not_singleton[of p] hA_sub_single by (by100 blast)
qed

lemma geotop_comb_boundary_edge_one_incident_2simplex_member_prefix:
  ...
  (**
    Direct unpacking of the combinatorial boundary definition: an edge with
    exactly one incident 2-simplex is one of the boundary 1-simplexes. **)
proof -
  let ?S = "{\<tau> \<in> K. geotop_simplex_dim \<tau> (2 - 1) \<and>
      card {\<sigma> \<in> K. geotop_simplex_dim \<sigma> 2 \<and>
        geotop_is_face \<tau> \<sigma>} = 1}"
  have he_dim: "geotop_simplex_dim e (2 - 1)"
    using hedge unfolding geotop_is_edge_def by (by100 simp)
  have heS: "e \<in> ?S"
    using heK he_dim hcard by (by100 simp)
  show ?thesis
    unfolding geotop_comb_boundary_def using heS by (by100 simp)
qed

lemma geotop_comb_boundary_edge_member_incident_2simplex_count_one_prefix:
  fixes K :: "(real^2) set set" and e :: "(real^2) set"
  assumes heBd: "e \<in> geotop_comb_boundary K 2"
  assumes hedge: "geotop_is_edge e"
  shows "card {\<sigma>\<in>K. geotop_simplex_dim \<sigma> 2 \<and> geotop_is_face e \<sigma>} = 1"
  (**
    Converse unpacking of the combinatorial boundary definition for edges:
    an edge lying in the 2-dimensional combinatorial boundary is itself the
    selected boundary edge, not merely a proper face of another boundary edge. **)
proof -
  let ?S = "{\<tau> \<in> K. geotop_simplex_dim \<tau> (2 - 1) \<and>
      card {\<sigma> \<in> K. geotop_simplex_dim \<sigma> 2 \<and>
        geotop_is_face \<tau> \<sigma>} = 1}"
  have he_cases:
    ...
    using heBd unfolding geotop_comb_boundary_def by (by100 simp)
  show ?thesis
...
lemma geotop_one_incident_edge_subset_frontier_polyhedron_prefix:
  ...
  assumes hcard1:
    "card {\<sigma>\<in>K. geotop_simplex_dim \<sigma> 2 \<and> geotop_is_face e \<sigma>} = 1"
  assumes hF: "F = frontier (geotop_polyhedron K)"
  shows "e \<subseteq> F"
  (**
    Moise 4.4 frontier-neighborhood bookkeeping: an edge with exactly one
    incident 2-simplex lies in the polyhedral frontier.  The local D44 uses
    this with \<open>F = FrN\<^sub>I\<close> after identifying \<open>N\<^sub>I\<close> with \<open>|K\<^sub>N|\<close>. **)
proof -
  obtain \<sigma> where hfaces:
      "{\<rho>\<in>K. geotop_simplex_dim \<rho> 2 \<and> geotop_is_face e \<rho>} = {\<sigma>}"
    using hcard1 by (rule card_1_singletonE)
  have h\<sigma>in: "\<sigma> \<in> {\<rho>\<in>K. geotop_simplex_dim \<rho> 2 \<and> geotop_is_face e \<rho>}"
  ...
  have h\<sigma>face: "geotop_is_face e \<sigma>"
    using h\<sigma>in by (by100 simp)
  have hrel_front:
      "rel_interior e \<subseteq> frontier (geotop_polyhedron K)"
  ...
  have hfront_closed: "closed (frontier (geotop_polyhedron K))"
  ...
  have he_front: "e \<subseteq> frontier (geotop_polyhedron K)"
    using hclosure_sub hclosure_e by (by100 simp)
  show ?thesis
    using he_front hF by (by100 simp)
qed

lemma geotop_two_incident_edge_rel_interior_subset_polyhedron_interior_prefix:
  fixes K :: "(real^2) set set" and e N :: "(real^2) set"
  assumes hK: "geotop_is_complex K"
  ...
  shows "rel_interior e \<subseteq> interior N"
  (**
    Moise 4.4 brick-neighborhood interior step: if two 2-bricks of the
    selected carrier meet along edge \<open>e\<close>, then the relative interior of
    \<open>e\<close> is not on the frontier of the carrier but lies in its interior. **)
proof -
  ...
  have hunion_sub_K: "\<sigma> \<union> \<tau> \<subseteq> geotop_polyhedron K"
  proof
    fix x
  ...
  qed
  have hrel_int_K:
      "rel_interior e \<subseteq> interior (geotop_polyhedron K)"
  proof -
    have hinter_sub:
        "interior (\<sigma> \<union> \<tau>) \<subseteq> interior (geotop_polyhedron K)"
      by (rule interior_mono[OF hunion_sub_K])
    show ?thesis
      using hrel_int_union hinter_sub by (by100 blast)
  qed
  show ?thesis
    using hrel_int_K hK_poly by (by100 simp)
qed

lemma geotop_two_incident_edge_rel_interior_disj_restricted_frontier_prefix:
  ...
  assumes hcard2:
    "card {\<sigma>\<in>K. geotop_simplex_dim \<sigma> 2 \<and> geotop_is_face e \<sigma>} = 2"
  shows "rel_interior e \<inter> Fr = {}"
  (**
    Continuation of the Moise 4.4 two-brick step: after restricting the
    selected carrier to the closed disk, the relative interior of an edge with
    two incident 2-bricks cannot meet the frontier. **)
proof -
  have hrel_int: "rel_interior e \<subseteq> interior N"
    by (rule geotop_two_incident_edge_rel_interior_subset_polyhedron_interior_prefix
        [OF hK hK_poly heK hedge hcard2])
  show ?thesis
  proof (rule ccontr)
    assume hne: "rel_interior e \<inter> Fr \<noteq> {}"
    obtain x where hx: "x \<in> rel_interior e \<inter> Fr"
      using hne by (by100 blast)
    have hx_int: "x \<in> interior N"
      using hx hrel_int by (by100 blast)
    have hx_front: "x \<in> frontier N"
      using hx hFr hN\<^sub>I by (by100 simp)
...
lemma geotop_edge_frontier_rel_interior_not_two_incident_restricted_prefix:
  fixes K :: "(real^2) set set" and e N N\<^sub>I Fr :: "(real^2) set"
    and p :: "real^2"
  assumes hK: "geotop_is_complex K"
  assumes hK_poly: "geotop_polyhedron K = N"
  ...
  shows "card {\<sigma>\<in>K. geotop_simplex_dim \<sigma> 2 \<and> geotop_is_face e \<sigma>} \<noteq> 2"
  (**
    If a point of the relative interior of edge \<open>e\<close> lies on the selected
    carrier frontier, \<open>e\<close> cannot be one of the two-brick interior edges. **)
proof
  assume hcard2:
    "card {\<sigma>\<in>K. geotop_simplex_dim \<sigma> 2 \<and> geotop_is_face e \<sigma>} = 2"
  have hdisj: "rel_interior e \<inter> Fr = {}"
    by (rule geotop_two_incident_edge_rel_interior_disj_restricted_frontier_prefix
        [OF hK hK_poly hFr hN\<^sub>I heK hedge hcard2])
  have "p \<in> rel_interior e \<inter> Fr"
...
lemma geotop_connected_linear_graph_exact_two_polyhedron_1sphere_prefix:
  ...
  (**
    Moise 4.4 graph bridge: exact-two local incidence on a finite connected
    linear graph makes its carrier polygonal, hence a 1-sphere. **)
proof -
  have hdegree:
      "\<forall>w. {w} \<in> L \<longrightarrow>
        card {e\<in>L. geotop_is_edge e \<and> w \<in> e} = 2"
    ...
    "geotop_is_n_sphere J\<^sub>N
     ...
     \<and> (\<forall>\<epsilon>\<^sub>Q>0. \<forall>\<epsilon>\<^sub>S>0.
        \<exists>B. geotop_is_broken_line B
          ...
          \<and> B \<inter> ball S1 \<epsilon>\<^sub>S \<noteq> {})"
  (**
    Moise 4.4 regular-neighborhood theorem to finish D44.  In the book,
    after choosing the sufficiently fine brick/fine-carrier neighborhood
    \<open>N\<close> of \<open>A1\<close> and restricting it to the closed disk, the frontier
...
proof -
  ...
  have hK\<^sub>N_two_incident_edge_rel_interior_subset_interior_N:
    ...
    by (rule geotop_two_incident_edge_rel_interior_subset_polyhedron_interior_prefix
        [OF hK\<^sub>N_complex hK\<^sub>N_poly])
  ...
  proof -
    assume hdegree:
      "\<forall>w. {w} \<in> BdJ\<^sub>N \<longrightarrow>
        card {e\<in>BdJ\<^sub>N. geotop_is_edge e \<and> w \<in> e} = 2"
    obtain u v C\<^sub>1 C\<^sub>2 where hpoly_eq:
        "geotop_polyhedron BdJ\<^sub>N = C\<^sub>1 \<union> C\<^sub>2"
  ...
  have hD44_J\<^sub>N_1sphere_imp_BdJ\<^sub>N_polygon:
    ...
    (**
      Moise's wording is about the frontier component \<open>J\<^sub>N\<close> itself being a
      1-sphere.  Since this package has already identified \<open>J\<^sub>N\<close> with the
      carrier of \<open>BdJ\<^sub>N\<close>, this is exactly the polygonal-carrier hypothesis
      needed by the finite graph degree-two classifier. **)
  proof -
    assume hsphere:
      "geotop_is_n_sphere J\<^sub>N
        (subspace_topology UNIV geotop_euclidean_topology J\<^sub>N) 1"
  ...
  have hD44_BdJ\<^sub>N_polygon_imp_degree_two:
      "geotop_is_polygon (geotop_polyhedron BdJ\<^sub>N) \<Longrightarrow>
        \<forall>w. {w} \<in> BdJ\<^sub>N \<longrightarrow>
          card {e\<in>BdJ\<^sub>N. geotop_is_edge e \<and> w \<in> e} = 2"
    (**
      Finite-graph form of Moise's regular-neighborhood boundary statement:
      once the carrier of the frontier boundary subcomplex is polygonal, the
      existing finite connected linear graph classifier gives degree two at
  ...
  have hD44_BdJ\<^sub>N_exact_two_incident_edges_imp_J\<^sub>N_1sphere:
    ...
    (**
      ...
      1-sphere.  The actual book construction proves the stronger exact-two
      frontier incidence at every vertex; the finite graph classifier already
      turns that local 1-manifold statement into polygonality of the carrier. **)
  proof -
    assume htwo:
      "\<forall>w. {w} \<in> BdJ\<^sub>N \<longrightarrow>
        (\<exists>e\<^sub>1\<in>BdJ\<^sub>N. \<exists>e\<^sub>2\<in>BdJ\<^sub>N.
    ...
    show ?thesis
      by (rule geotop_connected_linear_graph_exact_two_polyhedron_1sphere_prefix
          [OF hBdJ\<^sub>N_linear_graph hBdJ\<^sub>N_fin hBdJ\<^sub>N_nonempty
            hBdJ\<^sub>N_connected hJ\<^sub>N_eq_BdJ\<^sub>N_poly htwo])
  qed
  have hD44_regular_neighborhood_exact_two_and_component_book_step:
      ...
      \<and> S1 \<in> geotop_component_at UNIV geotop_euclidean_topology
            ?Ncut Q1"
    (**
      Literal Moise 4.4 regular-neighborhood content in the rich carrier
      context.  The component of the frontier through \<open>P\<close> is represented by
      the boundary subcomplex \<open>BdJ\<^sub>N\<close>; the book's sentence that it is a
      1-sphere is formalized here as exact-two incident boundary edges at every
  ...
  proof -
    have hD44_regular_neighborhood_frontier_sphere_and_route_book_step:
        "geotop_is_n_sphere J\<^sub>N
            (subspace_topology UNIV geotop_euclidean_topology J\<^sub>N) 1
        \<and> (\<forall>\<epsilon>\<^sub>Q>0. \<forall>\<epsilon>\<^sub>S>0.
          \<exists>B. geotop_is_broken_line B
            ...
            \<and> B \<inter> ball S1 \<epsilon>\<^sub>S \<noteq> {})"
      (**
        Single remaining Moise 4.4 regular-neighborhood step, in the book's
        own form.  After restricting the fine carrier to the closed disk, the
        frontier component through \<open>P\<close> is a 1-sphere.  Splitting that
        frontier 1-sphere into the boundary arc and the complementary
        lower-to-upper subarc gives the broken-line crossings of all access
        collars around \<open>Q1\<close> and \<open>S1\<close>. **)
      by (rule
        geotop_polygon_two_endpoint_arcs_regular_neighborhood_frontier_sphere_and_route_prefix
    ...
    have hsphere_book:
        "geotop_is_n_sphere J\<^sub>N
          (subspace_topology UNIV geotop_euclidean_topology J\<^sub>N) 1"
      using hD44_regular_neighborhood_frontier_sphere_and_route_book_step
      by (rule conjunct1)
    have hpolygon_book:
        "geotop_is_polygon (geotop_polyhedron BdJ\<^sub>N)"
      by (rule hD44_J\<^sub>N_1sphere_imp_BdJ\<^sub>N_polygon[OF hsphere_book])
    have hdegree:
        "\<forall>w. {w} \<in> BdJ\<^sub>N \<longrightarrow>
          card {e\<in>BdJ\<^sub>N. geotop_is_edge e \<and> w \<in> e} = 2"
      by (rule hD44_BdJ\<^sub>N_polygon_imp_degree_two[OF hpolygon_book])
    have htwo:
        "\<forall>w. {w} \<in> BdJ\<^sub>N \<longrightarrow>
          (\<exists>e\<^sub>1\<in>BdJ\<^sub>N. \<exists>e\<^sub>2\<in>BdJ\<^sub>N.
            geotop_is_edge e\<^sub>1 \<and> w \<in> e\<^sub>1
    ...
    have hsame:
        "S1 \<in> geotop_component_at UNIV geotop_euclidean_topology ?Ncut Q1"
      (**
        Moise 4.4, final corridor sentence: split the frontier circle into the
    ...
    have htwo:
        "\<forall>w. {w} \<in> BdJ\<^sub>N \<longrightarrow>
          (\<exists>e\<^sub>1\<in>BdJ\<^sub>N. \<exists>e\<^sub>2\<in>BdJ\<^sub>N.
            ...
            \<and> (\<forall>e. e \<in> BdJ\<^sub>N \<and> geotop_is_edge e \<and> w \<in> e
                \<longrightarrow> e = e\<^sub>1 \<or> e = e\<^sub>2))"
      using hD44_regular_neighborhood_exact_two_and_component_book_step
      by (rule conjunct1)
    have hsphere:
        "geotop_is_n_sphere J\<^sub>N
          (subspace_topology UNIV geotop_euclidean_topology J\<^sub>N) 1"
