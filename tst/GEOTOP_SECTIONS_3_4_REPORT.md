...
- A fetch of colleague `main` over HTTPS reached commit `81d9ff22` (`Export TFF
  problems for Top0 topology sessions`). The relevant faster theorem-index
...
- The fast `gen_index.sh` implementation is the Python single-pass version and
  ...
  ran in `0:00.46`, while `gen_stmt_index.sh` ran in `0:02.26`.
- A warm-cache section build passed after the latest Figure 4.10 polygon versus
  ...
  `0:38.29` elapsed time; the session itself reported `0:00:15 elapsed time`.
...
radial bad-endpoint closedness bottleneck. The full-boundary Figure 4.10
boundary-cone fan path is now restricted to polygonal links, matching the book's
interior-vertex case. Broken-line links now use a separate boundary-vertex fan
obligation: enumerate the broken-line link as an edge-chain, place it as a
subdivided boundary arc of a 2-simplex, and cone from the corresponding target
boundary vertex. The one-sided and three-sided chart contradictions are now
...
Figure 4.10 boundary model, the explicit cone-over-boundary-subdivision
...
- `geotop_fig410_boundary_subdivision_model_from_finite_linear_graph_polygon_dev34`
  at line 3188.
- `geotop_fig410_explicit_cone_over_boundary_subdivision_dev34` at line 3236.
...
- `geotop_three_incident_2simplex_small_circle_domain_not_separates_chart_dev34`
  at line 4506.
- `geotop_2cell_chart_1sphere_complement_not_connected_dev34` at line 4642.
- `geotop_standard_fan_target_vertex_HOL_interior_polyhedron_dev34` at line
  9323; this is the Figure 4.10 full-disk vertex-star local
...
- `geotop_vertex_star_standard_fan_isomorphism_from_finite_linear_link_polygon_dev34`
- `geotop_vertex_star_standard_fan_model_from_finite_linear_link_polygon_dev34`
...
The standard boundary-cone fan wrapper, the pure cone wrapper, the
isomorphism-transport cone wrapper, and the polygonal link-specific
...
2-simplex; prove the explicit cone-over-boundary-subdivision construction with
the cone complex named as `F` plus all cones over nonempty simplexes of `F`; and
prove the broken-line boundary-vertex analogue by placing the ordered
edge-chain as a boundary arc.
...
derives the cached finite 1-dimensional link data, invokes the polygon-only
standard fan target, and delegates only the final local-neighborhood transfer to
...
is also proved; it delegates to the analytic helper
`geotop_2simplex_opposite_side_shared_edge_rel_interior_subset_HOL_interior_union_dev34`,
which is now closed by the local Euclidean ball construction for two
opposite-side triangles along their common edge. The edge-coordinate
...
in that simplex. The containment package
`geotop_shared_edge_probe_triangles_subset_union_dev34` now applies this
convexity wrapper to the two small probe triangles used in the book's local
diamond argument, placing their union inside the two incident 2-simplexes.
The local diamond ball-containment helper
`geotop_shared_edge_probe_diamond_contains_ball_dev34` is now proved. It
reduces the ball-containment lemma to orthogonal coordinate spanning,
coordinate representation, small coordinate control near the center point, and
upper/lower triangle membership, then proves the final ball-subset bookkeeping
from those subclaims. The analytic wrapper
`geotop_2simplex_opposite_side_shared_edge_rel_interior_subset_HOL_interior_union_dev34`
is now also proved: it combines the shared-edge subsegment, the two normal
probes, the local diamond ball, and the convexity containment package to show
that every relative-interior point of the common edge is in the HOL interior of
the union of the two opposite-side 2-simplexes.
...
- The next book-aligned bottleneck for Theorems 4.8 and 4.9 is the Figure 4.10
  ...
  `geotop_fig410_boundary_subdivision_model_from_finite_linear_graph_polygon_dev34`,
  `geotop_fig410_explicit_cone_over_boundary_subdivision_dev34`, and
  `geotop_vertex_star_fan_model_from_finite_linear_link_broken_line_dev34`.
...
- For Theorem 4.9's boundary converse, the two-triangle shared-edge
  neighborhood branch is proved; the remaining local Euclidean-neighborhood
  branch is the polygonal-link vertex-star disk model.
