...
lemma geotop_connected_trace_component_closure_touch_three_sets_exists_prefix:
  fixes A U C G\<^sub>1 G\<^sub>2 G\<^sub>3 :: "'a::real_normed_vector set"
  assumes hC: "C \<in> components U"
  assumes hA_conn: "connected A"
  assumes hA_sub: "A \<subseteq> U"
  ...
  assumes hG\<^sub>2: "G\<^sub>2 \<inter> closure A \<noteq> {}"
  assumes hG\<^sub>3: "G\<^sub>3 \<inter> closure A \<noteq> {}"
  shows "\<exists>D. D \<in> components U
    \<and> G\<^sub>1 \<inter> closure D \<noteq> {}
    \<and> G\<^sub>2 \<inter> closure D \<noteq> {}
    \<and> G\<^sub>3 \<inter> closure D \<noteq> {}"
proof -
  have htouch_C:
      "G\<^sub>1 \<inter> closure C \<noteq> {}
        \<and> G\<^sub>2 \<inter> closure C \<noteq> {}
        \<and> G\<^sub>3 \<inter> closure C \<noteq> {}"
    by (rule geotop_connected_trace_inside_component_closure_touch_three_sets_prefix
        [OF hC hA_conn hA_sub hA_meets_C hG\<^sub>1 hG\<^sub>2 hG\<^sub>3])
  show ?thesis
    using hC htouch_C by (intro exI conjI)
qed
...
lemma geotop_finite_trace_component_closure_touch_summary_prefix:
  fixes F :: "'a::topological_space set set"
    and U G :: "'a set"
  ...
  assumes hsummary:
    "\<And>C. C \<in> F
      \<Longrightarrow> C \<in> components U
        \<and> C \<noteq> {}
        \<and> C \<subseteq> U
        \<and> connected C
...
proof -
  obtain C where hC: "C \<in> F" and htouch_C: "G \<inter> closure C \<noteq> {}"
    using geotop_finite_union_closure_touch_member_prefix[OF hF htouch]
    by (elim bexE)
  have hC_summary:
      "C \<in> components U
      \<and> C \<noteq> {}
  ...
  show ?thesis
    using hC hC_summary htouch_C by (by100 blast)
qed
  ...
  proof -
    ...
    proof -
          ...
          proof -
            ...
            proof -
              ...
              proof -
                ...
                qed
              qed
              have hW_source_ball_entry_cover_cases:
                  "((\<exists>D a. N = D - {w}
                      \<and> W = D - {w, p}
                      \<and> y \<in> D - {p}
                      \<and> w \<in> closure W
              ...
              proof -
                show ?thesis
                  using hW_source_ball_entry_cases
                proof (elim disjE exE conjE)
                  fix D
                  assume hN_eq: "N = D - {w}"
                  assume hW_eq: "W = D - {w, p}"
                  assume hyD: "y \<in> D - {p}"
                  assume hwW_cl: "w \<in> closure W"
                  assume hW_ball: "W \<inter> ball w r \<noteq> {}"
                  obtain a where ha: "a \<in> W \<inter> ball w r"
                    using hW_ball by (by100 blast)
                  have ha_cover:
                      "a \<in> ((S - {w}) \<inter> ball w r)
                        \<union> ((T - {w}) \<inter> ball w r)
                        \<union> ((U - {w}) \<inter> ball w r)
                        \<union> \<Union>?Ntrace_components"
                    using hW_ball_cover_by_selected_and_trace_components ha
                    by (by100 blast)
                  show ?thesis
                  proof (rule disjI1)
                    ...
                    proof (rule exI[where x=D], rule exI[where x=a])
                      show "N = D - {w}
                        ...
                        \<and> a \<in> ((S - {w}) \<inter> ball w r
                          ...
                          \<union> \<Union>?Ntrace_components)"
                      proof (intro conjI)
                        show "N = D - {w}" by (rule hN_eq)
                        show "W = D - {w, p}" by (rule hW_eq)
                        show "y \<in> D - {p}" by (rule hyD)
                        show "w \<in> closure W" by (rule hwW_cl)
                        show "a \<in> W \<inter> ball w r" by (rule ha)
                        show "a \<in> ((S - {w}) \<inter> ball w r
                          \<union> ((T - {w}) \<inter> ball w r)
                          \<union> ((U - {w}) \<inter> ball w r)
                          \<union> \<Union>?Ntrace_components)"
                ...
                qed
              qed
              have hW_source_ball_entry_refined_cases:
                  "((\<exists>D a. N = D - {w}
                      \<and> W = D - {w, p}
                      \<and> y \<in> D - {p}
                      \<and> w \<in> closure W
              ...
              proof -
                show ?thesis
                  using hW_source_ball_entry_cover_cases
                proof (elim disjE exE conjE)
                  fix D a
                  assume hN_eq: "N = D - {w}"
                  assume hW_eq: "W = D - {w, p}"
              ...
              proof -
                ...
                qed
              qed
              have hW_source_ball_entry_named_cases:
                  "((\<exists>D. N = D - {w}
                      \<and> W = D - {w, p}
                      \<and> y \<in> D - {p}
                      \<and> w \<in> closure W
              ...
              proof -
                show ?thesis
                  using hW_source_ball_entry_trace_component_cases
                proof (elim disjE)
                  assume hleft: "\<exists>D a. N = D - {w}
                    \<and> W = D - {w, p}
                    \<and> y \<in> D - {p}
                      ...
                      show "N = D - {w}
                        ...
                        \<and> ((\<exists>a. a \<in> W \<inter> ball w r
                          ...
                          \<or> (\<exists>C\<in>?Ntrace_components.
                            C \<in> components ?Lcomp
                            \<and> W \<inter> ball w r \<inter> C \<noteq> {}))"
                        using hN_eq hW_eq hyD hwW_cl hentry_named by blast
                    qed
                  qed
                next
                  assume hright: "\<exists>D. N = D
              ...
              proof (intro conjI)
                ...
                show "((\<exists>D a. N = D - {w}
                  ...
                  \<or> (\<exists>D. N = D
                    ...
                    \<and> y \<in> D - {p}
                    \<and> q\<^sub>1 \<in> closure W))"
                  by (rule hW_source_ball_entry_cover_cases)
                show "((\<exists>D a. N = D - {w}
                    \<and> W = D - {w, p}
                    \<and> y \<in> D - {p}
                    \<and> w \<in> closure W
              ...
              have hW_local_trace_component_touch_summary:
                  "W \<inter> ?Lcomp \<noteq> {}
                    \<Longrightarrow> \<exists>C\<in>?Ntrace_components.
                      ...
                      \<and> W \<inter> closure C \<noteq> {}"
              proof -
                assume hWloc: "W \<inter> ?Lcomp \<noteq> {}"
                obtain a where haW: "a \<in> W" and haL: "a \<in> ?Lcomp"
                  using hWloc by (by100 blast)
                have ha_union: "a \<in> \<Union>?Ntrace_components"
                proof -
                  have "a \<in> W \<inter> ?Lcomp"
                    using haW haL by (by100 blast)
                  thus ?thesis
                    using hW_local_trace_component_cover by (by100 blast)
                qed
                obtain C where hC: "C \<in> ?Ntrace_components"
                  and haC: "a \<in> C"
                  using ha_union by (by100 blast)
                have ha_ball: "a \<in> ball w r"
                  using haL by (by100 blast)
                have hW_meets_C: "W \<inter> ball w r \<inter> C \<noteq> {}"
                  using haW ha_ball haC by (by100 blast)
                have hsummary:
                    "C \<in> components ?Lcomp
                      \<and> C \<noteq> {}
                      \<and> C \<subseteq> ?Lcomp
              ...
              proof -
                ...
                qed
              qed
              have hW_source_ball_entry_flat_summary_cases:
                  "(\<exists>D a. N = D - {w}
                      \<and> W = D - {w, p}
                      \<and> y \<in> D - {p}
                      \<and> w \<in> closure W
              ...
              proof -
                show ?thesis
                  using hW_source_ball_entry_named_summary_cases
                proof (elim disjE)
                  assume hleft: "\<exists>D. N = D - {w}
                    \<and> W = D - {w, p}
                    \<and> y \<in> D - {p}
                  ...
                  proof (elim disjE)
                    assume hS_entry:
                        "\<exists>a. a \<in> W \<inter> ball w r
                          \<and> a \<in> ((S - {w}) \<inter> ball w r)"
                    then obtain a where haW: "a \<in> W \<inter> ball w r"
                    ...
                    proof (rule disjI1)
                      ...
                      proof (rule exI[where x=D], rule exI[where x=a])
                        ...
                        proof (intro conjI)
                          ...
                          show "y \<in> D - {p}" by (rule hyD)
                          show "w \<in> closure W" by (rule hwW_cl)
                          show "a \<in> W \<inter> ball w r" by (rule haW)
                          show "a \<in> ((S - {w}) \<inter> ball w r)"
                            by (rule haS)
                        qed
                      qed
                  ...
                  next
                    ...
                    proof (rule disjI2, rule disjI2, rule disjI2, rule disjI1)
                      show "\<exists>D C. N = D - {w}
                        ...
                        \<and> W \<inter> ball w r \<inter> C \<noteq> {}
                        \<and> W \<inter> closure C \<noteq> {}"
                      proof (rule exI[where x=D], rule exI[where x=C])
                        show "N = D - {w}
                          \<and> W = D - {w, p}
                          \<and> y \<in> D - {p}
                          \<and> w \<in> closure W
                ...
                proof (elim disjE)
                  ...
                  proof (rule disjI1)
                    show "\<exists>D. N = D - {w}
                      ...
                      \<and> w \<in> closure W
                      \<and> ((S - {w}) \<inter> ball w r) \<inter> closure W \<noteq> {}"
                      using hN_eq hW_eq hyD hwW_cl htouch by (by100 blast)
                  qed
                next
                  assume hT:
                      "\<exists>D a. N = D - {w}
              ...
              proof -
                ...
                show "\<exists>D. D \<in> components ?Lcomp
                  ...
                  \<and> ((T - {w}) \<inter> ball w r) \<inter> closure D \<noteq> {}
                  \<and> ((U - {w}) \<inter> ball w r) \<inter> closure D \<noteq> {}"
                  by (rule geotop_connected_trace_component_closure_touch_three_sets_exists_prefix
      ...
      show "\<exists>C. C \<in> components (ball w r - (e\<^sub>1 \<union> e\<^sub>2 \<union> e\<^sub>3))
        ...
        \<and> (T - {w}) \<inter> ball w r \<inter> closure C \<noteq> {}
        \<and> (U - {w}) \<inter> ball w r \<inter> closure C \<noteq> {}"
        by (rule hsplit_side_endpoint_local_component_book_step)
    qed
    show ?thesis
    proof -
      obtain S T U p y z N D\<^sub>w D\<^sub>q where hS_E: "S \<in> E"
