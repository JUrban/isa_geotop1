...
isolated in a cached development stack, and the current active session has a
successful warm-cache build.

Evidence checked locally:
\begin{itemize}
\item A pull/fetch of colleague \texttt{main} over HTTPS left \texttt{https/main} at
\texttt{3e463c3b} (\texttt{Document GeoTop sections 1 and 2 status}).
...
reports \texttt{224 0}, so no merge was needed for this report update.
...
\item That build passed for the state committed as \texttt{ce1df618}
(\texttt{Decompose GeoTop one-sided chart lemma}); the final
\texttt{GeoTop34Dev} theory took about \texttt{0:00:09} elapsed, with about
\texttt{0:00:29} elapsed overall.
...
\begin{itemize}
\item \texttt{geotop\_link\_open\_radial\_cone\_open\_in\_punctured\_star\_dev34} at line 117.
\item \texttt{geotop\_vertex\_star\_cone\_equiv\_from\_link\_line\_or\_polygon\_dev34} at line 1223.
\item The semicircle-separation subclaim inside \texttt{geotop\_unique\_incident\_2simplex\_semicircle\_separates\_chart\_dev34} at line 1306.
\item \texttt{geotop\_three\_incident\_2simplex\_sphere\_not\_separates\_chart\_dev34} at line 1333.
\item \texttt{geotop\_boundary\_chart\_three\_incident\_2simplex\_contradiction\_dev34} at line 1356.
\item The boundary equality half of \texttt{Theorem\_GT\_4\_9} at line 3110.
\end{itemize}
...
\texttt{geotop\_2\_manifold\_vertex\_star\_punctured\_connected\_dev34}, and
\texttt{geotop\_2\_manifold\_with\_boundary\_vertex\_star\_punctured\_connected\_dev34}.
The radial endpoint projection is now proved from
\texttt{geotop\_punctured\_star\_radial\_endpoint\_choice\_property\_dev34},
\texttt{geotop\_radial\_endpoint\_choice\_preimage\_eq\_cone\_dev34}, and an
explicit cone-openness subclaim.
...
\texttt{openin\_on} chart-neighborhood hypothesis, the helper
\texttt{geotop\_openin\_norm\_polyhedron\_contains\_relative\_ball\_dev34}
extracts a relative metric ball from that hypothesis, and the one-sided chart
lemma has been decomposed so that its remaining hole is the explicit
semicircle separation construction inside the local neighborhood.
...
\texttt{geotop\_plane\_chart\_point\_complement\_connected}, and
...
bridge at line 1223 should be the next bottleneck. The local chart
contradiction lemmas at lines 1306, 1333, and 1356 now have the needed
local-open-neighborhood hypothesis; the next step there is to formalize the
