...
This should not be over-read as a finished endpoint target. The current
boundary-subdivision bridge is useful, but one version assumes the target chain
is a subdivision of the full 2-simplex boundary; the endpoint theorem needs a
true boundary-arc chain on one side of the triangle, then the cone/fan
construction from the remaining vertex. The remaining target construction is
therefore still: produce a same-length endpoint chain `F,w',q',us` on a
standard boundary arc, plus the indexwise vertex bijection and the cone clauses.
...
theorems or report/advice notes. The two index scripts already use
`index_theory_lib.py` and include active theories, local imports,
advice/report notes, session/signature files, bounded session logs, and ROOT
files in their cache signatures. I do not see an immediate script edit needed
for the current session files; the important action is to rerun both scripts
