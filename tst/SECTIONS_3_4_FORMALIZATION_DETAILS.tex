...
\newcommand{\bookexcerpt}[2]{%
  \paragraph{Book LaTeX source.}%
  \lstinputlisting[firstnumber=#1]{sections_3_4_snippets/#2}%
  \paragraph{Book rendered excerpt.}%
  \begin{quote}\small\input{sections_3_4_rendered/#2}\end{quote}%
}
...
\begin{itemize}
  \item the exact book LaTeX source range from \src{geotop.tex}, including the
    book statement and proof text for that item;
  \item a rendered book excerpt from the same source range, with figure
    environments replaced by labeled placeholders when the original image asset
...
The exact source excerpts are stored as small pre-extracted files under
\src{sections_3_4_snippets/}; rendered book excerpts are stored under
\src{sections_3_4_rendered/}.  Each raw listing uses \verb|firstnumber| to
preserve the original source line numbers.  This avoids repeated
high-line-number scans through the large Isabelle theory files during PDF
generation.
...
\bookexcerpt{752}{book_free_2_simplex.tex}
...
\bookexcerpt{943}{book_brick_decomposition.tex}
...
\bookexcerpt{1058}{book_4_10.tex}
