...
text \<open>Bridge lemma: any topology generated by a basis is automatically strict,
  since the definition of topology\_generated\_by\_basis includes U ⊆ X.\<close>
lemma topology_generated_by_basis_is_strict:
  assumes "is_basis_on X B"
  shows "is_topology_on_strict X (topology_generated_by_basis X B)"
  unfolding is_topology_on_strict_def
proof (intro conjI)
  show "is_topology_on X (topology_generated_by_basis X B)"
    by (rule topology_generated_by_basis_is_topology_on[OF assms])
  show "topology_generated_by_basis X B \<subseteq> Pow X"
...
text \<open>Corollary: subspace topologies inherit strictness.\<close>
lemma subspace_topology_is_strict:
  assumes hstrict: "is_topology_on_strict X T" and hAX: "A \<subseteq> X"
  shows "is_topology_on_strict A (subspace_topology X T A)"
  unfolding is_topology_on_strict_def
proof (intro conjI)
  show "is_topology_on A (subspace_topology X T A)"
    by (rule subspace_topology_is_topology_on[OF is_topology_on_strict_imp[OF hstrict] hAX])
  show "subspace_topology X T A \<subseteq> Pow A"
    unfolding subspace_topology_def by blast
qed
...
text \<open>Metric topologies are strict: every open set is a subset of X.\<close>
lemma metric_topology_is_strict:
